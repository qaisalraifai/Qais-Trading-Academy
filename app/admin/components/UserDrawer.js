...
function Row({ label, value }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", padding: "0.5rem 0", fontSize: "0.85rem" }}>
      <span style={{ color: "#5D6880" }}>{label}</span>
      <span style={{ color: "#EDF1F8", fontFamily: monoStack, fontSize: "0.82rem" }}>{value ?? "—"}</span>
    </div>
  );
}
...
function SectionDivider({ children }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", margin: "1.3rem 0 0.5rem" }}>
      <span style={{ fontSize: "0.72rem", color: "#3E4761", fontWeight: 600, letterSpacing: "0.5px" }}>{children}</span>
      <div style={{ flex: 1, height: 1, background: "#1E2941" }} />
    </div>
  );
}
  ...
  return (
    <>
      ...
      >
        ...
        <button onClick={onClose} style={{ background: "none", border: "1px solid #1B2438", color: "#5D6880", borderRadius: 3, width: 32, height: 32, cursor: "pointer", marginBottom: "1rem" }}>✕</button>

        {loading || !p ? (
          <p style={{ color: "#3E4761", textAlign: "center", padding: "3rem 0" }}>جاري التحميل...</p>
        ) : (
          <>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "0.6rem", padding: "0.5rem 0 1rem" }}>
              <div style={{ width: 72, height: 72, borderRadius: "50%", background: "linear-gradient(135deg,#111726,#111726)", border: `2px solid ${gold}55`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.8rem", color: gold, fontWeight: 700, overflow: "hidden" }}>
            ...
            {editing ? (
              <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                {["username", "email", "phone", "country"].map((f) => (
                  <input key={f} placeholder={f} value={form[f] || ""} onChange={(e) => setForm({ ...form, [f]: e.target.value })} style={{ background: "#080B14", border: "1px solid #1B2438", color: "#EDF1F8", padding: "0.5rem 0.7rem", borderRadius: 3, fontSize: "0.85rem" }} />
                ))}
                <select value={form.plan} onChange={(e) => setForm({ ...form, plan: e.target.value })} style={{ background: "#080B14", border: "1px solid #1B2438", color: "#EDF1F8", padding: "0.5rem 0.7rem", borderRadius: 3, fontSize: "0.85rem" }}>
            ...
            {data.payments?.length ? (
              <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
                {data.payments.slice(0, 8).map((pay) => (
                  <div key={pay.id} style={{ display: "flex", justifyContent: "space-between", fontSize: "0.8rem", padding: "0.4rem 0", borderBottom: "1px solid #111726" }}>
                    <span style={{ color: "#93A0B8" }}>{new Date(pay.created_at).toLocaleDateString("ar")}</span>
                    <span style={{ color: pay.status === "paid" ? "#1FBF87" : "#E8495F" }}>{pay.status}</span>
                    <span style={{ fontFamily: monoStack, color: gold }}>${Number(pay.amount).toFixed(2)}</span>
                  </div>
                ))}
              </div>
            ) : <p style={{ color: "#3E4761", fontSize: "0.8rem" }}>لا يوجد دفعات مسجلة</p>}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.55rem", maxHeight: 220, overflowY: "auto" }}>
              {data.activity?.length ? data.activity.map((a) => (
                <div key={a.id} style={{ display: "flex", gap: "0.5rem", fontSize: "0.8rem" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ color: "#93A0B8" }}>{a.message}</div>
                    <div style={{ color: "#3E4761", fontSize: "0.7rem" }}>{timeAgo(a.created_at)}</div>
                  </div>
                </div>
              )) : <p style={{ color: "#3E4761", fontSize: "0.8rem" }}>لا يوجد أنشطة مسجلة</p>}
            </div>
              ...
              {editing ? (
                <>
                  <button
                    ...
                    style={{ background: `linear-gradient(135deg, ${gold}, #9C7F42)`, color: "#000", border: "none", padding: "0.55rem 1.1rem", borderRadius: 3, cursor: "pointer", fontWeight: 700, fontSize: "0.82rem" }}
                  >
                    حفظ
                  </button>
                  <button onClick={() => setEditing(false)} style={{ background: "none", border: "1px solid #1B2438", color: "#93A0B8", padding: "0.55rem 1.1rem", borderRadius: 3, cursor: "pointer", fontSize: "0.82rem" }}>إلغاء</button>
                </>
              ) : (
                <>
                  <button onClick={() => setEditing(true)} style={actionBtnStyle()}>Edit</button>
                  {p.role !== "admin" && (
                    <button onClick={() => onAction("activate_free", p)} style={actionBtnStyle("#1FBF87")}>تفعيل مجاني</button>
                  )}
                  ...
                  <button onClick={() => onAction("delete", p)} style={actionBtnStyle("#E8495F")}>Delete</button>
                </>
              )}
            </div>
          </>
...
function actionBtnStyle(color = "#93A0B8") {
  return {
    background: "none",
