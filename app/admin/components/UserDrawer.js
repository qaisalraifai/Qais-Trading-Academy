  ...
  return (
    <>
      ...
      >
        ...
        <button onClick={onClose} style={{ background: "none", border: "1px solid #222", color: "#888", borderRadius: 8, width: 32, height: 32, cursor: "pointer", marginBottom: "1rem" }}>✕</button>

        {loading || !p ? (
          <p style={{ color: "#555", textAlign: "center", padding: "3rem 0" }}>جاري التحميل...</p>
        ) : (
          <>
            ...
            {data.payments?.length ? (
              <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
                {data.payments.slice(0, 8).map((pay) => (
                  <div key={pay.id} style={{ display: "flex", justifyContent: "space-between", fontSize: "0.8rem", padding: "0.4rem 0", borderBottom: "1px solid #141414" }}>
              ...
              {data.activity?.length ? data.activity.map((a) => (
                <div key={a.id} style={{ display: "flex", gap: "0.5rem", fontSize: "0.8rem" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ color: "#444", fontSize: "0.7rem" }}>{timeAgo(a.created_at)}</div>
                  </div>
                </div>
            ...
            <div style={{ display: "flex", gap: "0.6rem", flexWrap: "wrap", marginTop: "0.5rem" }}>
              {editing ? (
                <>
                  ...
                  <button onClick={() => setEditing(false)} style={{ background: "none", border: "1px solid #222", color: "#999", padding: "0.55rem 1.1rem", borderRadius: 10, cursor: "pointer", fontSize: "0.82rem" }}>إلغاء</button>
                </>
              ) : (
                <>
...
function actionBtnStyle(color = "#999") {
  return {
    background: "none",
