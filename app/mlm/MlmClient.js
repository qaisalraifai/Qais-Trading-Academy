  ...
  return (
    <div
      style={{
        flex: 1,
        background: "#0C1220",
        border: `1px dashed ${child ? GOLD + "55" : "#26314A"}`,
        borderRadius: 0,
        padding: "1rem",
        textAlign: "center",
      }}
    >
      <div style={{ fontSize: "0.7rem", color: "#5D6880", marginBottom: 6 }}>{label}</div>
      {child ? (
        <>
          <div style={{ fontWeight: 700 }}>{child.username}</div>
          <div style={{ fontSize: "0.7rem", color: child.is_active_member ? "#1FBF87" : "#5D6880", marginTop: 4 }}>
            {child.is_active_member ? t("mlm.active") : t("mlm.inactive")}
          </div>
        </>
      ) : (
        <div style={{ color: "#3E4761", fontSize: "0.85rem" }}>{t("mlm.emptySlot")}</div>
      )}
    </div>
  );
}
...
export default function MlmClient({ embedded = false }) {
  ...
  const outerStyle = embedded
    ? { color: "#EDF1F8", direction: dir }
    : { color: "#EDF1F8", padding: "2rem 1.5rem 3rem", direction: dir, maxWidth: 1150, margin: "0 auto" };

  if (loading) {
    return (
      <div style={embedded ? { color: "#EDF1F8", direction: dir } : { background: BG, color: "#EDF1F8", minHeight: "100vh", padding: "3rem", direction: dir }}>
        {t("mlm.loading")}
      </div>
    );
  }
  ...
  if (error || !data) {
    return (
      <div style={embedded ? { color: "#E8495F", direction: dir } : { background: BG, color: "#E8495F", minHeight: "100vh", padding: "3rem", direction: dir }}>
    ...
    <div style={outerStyle}>
      ...
      <Card style={{ marginBottom: "1.5rem" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "1rem" }}>
          <div>
            <div style={{ fontSize: "0.75rem", color: "#5D6880" }}>{t("mlm.currentRank")}</div>
            <div style={{ fontSize: "1.4rem", fontWeight: 800, color: GOLD }}>{rank?.name_ar || t("mlm.noRank")}</div>
          </div>
          <div style={{ textAlign: "left" }}>
            <div style={{ fontSize: "0.75rem", color: profile.isActiveMember ? "#1FBF87" : "#E8495F" }}>
              {profile.isActiveMember ? t("mlm.activeMembership") : t("mlm.inactiveMembership")}
            </div>
            <div style={{ fontSize: "0.7rem", color: "#5D6880", marginTop: 2 }}>
        ...
        {nextRank && (
          <div style={{ marginTop: "1.2rem", paddingTop: "1.2rem", borderTop: `1px solid ${BORDER}` }}>
            <div style={{ fontSize: "0.75rem", color: "#5D6880", marginBottom: 6 }}>
            ...
            <div style={{ display: "flex", gap: 6 }}>
              <div style={{ flex: 1, height: 6, borderRadius: 3, background: "#1E2941", overflow: "hidden" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: "1rem", marginBottom: "1.5rem" }}>
        {[
          ...
          <Card key={label}>
            ...
            <div style={{ fontSize: "1.3rem", fontWeight: 800 }}>{fmt(value)} <span style={{ fontSize: "0.75rem", color: "#5D6880" }}>{t("mlm.currencyUnit")}</span></div>
          </Card>
        ))}
      </div>
      ...
      <Card style={{ marginBottom: "1.5rem" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
          <div style={{ fontSize: "0.85rem", color: "#5D6880" }}>{t("mlm.myBinaryTree")}</div>
        ...
        <div style={{ display: "flex", gap: "1rem", marginTop: "1rem", fontSize: "0.8rem", color: "#5D6880" }}>
          <div>{t("mlm.cvLeft")} <strong style={{ color: "#EDF1F8" }}>{fmt(profile.cvLeft)}</strong></div>
          <div>{t("mlm.cvRight")} <strong style={{ color: "#EDF1F8" }}>{fmt(profile.cvRight)}</strong></div>
          <div>{t("mlm.carryUnmatched")} <strong style={{ color: "#EDF1F8" }}>{fmt(profile.carryLeft)} / {fmt(profile.carryRight)}</strong></div>
        </div>
      </Card>

      {/* KYC */}
      <Card style={{ marginBottom: "1.5rem" }}>
        <div style={{ fontSize: "0.85rem", color: "#5D6880", marginBottom: "1rem" }}>{t("mlm.kycTitle")}</div>
        {kycStatus === "verified" ? (
          <div style={{ color: "#1FBF87" }}>{t("mlm.kycVerified")}</div>
        ) : kycStatus === "pending" ? (
          ...
          <form onSubmit={submitKyc} style={{ display: "flex", gap: "0.8rem", alignItems: "center", flexWrap: "wrap" }}>
            ...
            <button type="submit" disabled={!kycFile || kycUploading}
              style={{ background: GOLD, color: "#111726", border: "none", borderRadius: 3, padding: "0.5rem 1.2rem", fontWeight: 700, cursor: "pointer" }}>
              {kycUploading ? t("mlm.kycUploading") : t("mlm.kycSubmitBtn")}
            </button>
            {kycStatus === "rejected" && <span style={{ color: "#E8495F", fontSize: "0.8rem" }}>{t("mlm.kycRejected")}</span>}
          </form>
        )}
      ...
      {/* طلب سحب */}
      <Card style={{ marginBottom: "1.5rem" }}>
        <div style={{ fontSize: "0.85rem", color: "#5D6880", marginBottom: "1rem" }}>
          {t("mlm.withdrawTitle", { balance: fmt(wallets.withdrawal) })}
        </div>
        {kycStatus !== "verified" ? (
          <div style={{ color: "#5D6880", fontSize: "0.85rem" }}>{t("mlm.withdrawNeedsKyc")}</div>
        ) : (
          <form onSubmit={submitWithdraw} style={{ display: "flex", gap: "0.8rem", flexWrap: "wrap", alignItems: "center" }}>
            <input type="number" min="50" step="0.01" placeholder={t("mlm.amountPlaceholder")} value={withdrawAmount}
              ...
              style={{ width: 120, padding: "0.5rem", borderRadius: 3, border: "1px solid #26314A", background: "#0C1220", color: "#EDF1F8" }} />
            <select value={withdrawMethod} onChange={(e) => setWithdrawMethod(e.target.value)}
              style={{ padding: "0.5rem", borderRadius: 3, border: "1px solid #26314A", background: "#0C1220", color: "#EDF1F8" }}>
            ...
            <input placeholder={t("mlm.accountNumberPlaceholder")} value={withdrawDest} onChange={(e) => setWithdrawDest(e.target.value)}
              style={{ flex: 1, minWidth: 180, padding: "0.5rem", borderRadius: 3, border: "1px solid #26314A", background: "#0C1220", color: "#EDF1F8" }} />
      ...
      {/* آخر العمولات */}
      <Card>
        <div style={{ fontSize: "0.85rem", color: "#5D6880", marginBottom: "1rem" }}>{t("mlm.recentCommissionsTitle")}</div>
        {recentCommissions.length === 0 ? (
          <div style={{ color: "#3E4761", fontSize: "0.85rem" }}>{t("mlm.noCommissionsYet")}</div>
        ) : (
          <div style={{ display: "grid", gap: "0.6rem" }}>
            {recentCommissions.map((c, i) => (
              ...
              >
                ...
                <span style={{ color: "#5D6880" }}>{new Date(c.created_at).toLocaleDateString(dateLocale)}</span>
              </div>
            ))}
          </div>
        )}
