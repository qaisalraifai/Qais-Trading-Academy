...
function sanitizeCandles(list) {
  ...
  const clean = list
    ...
    // نصلّح ترتيب أعلى/أقل سعر منطقياً حتى لو مصدر البيانات رجّع H/L مقلوبين أو ناقصين
    // (بيسبب هاد بالذات كراش بمكتبة الشارت لأنها بتفترض دايماً high >= open/close/low)
    .map((c) => ({
      ...c,
...
export default function ReplayClient({ userId }) {
  ...
  const loadGenRef = useRef(0); // رقم جيل التحميل — يتزايد كل ما بتبدأ عملية تحميل بيانات جديدة،
  // يستخدم لتجاهل أي استجابة شبكة متأخرة (وصلت بعد ما المستخدم بدّل فريم/أصل) بدل ما تلخبط الشارت
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (loadGenRef.current !== myGen) return; // نفس الشي: خطأ من طلب قديم متجاوَز، نتجاهله
  ...
  useEffect(() => {
    ...
    try {
      ...
      // بيانات فاسدة وصلت رغم التصفية (مصدر خارجي غير متوقع) - قبل ما نستسلم ونعرض رسالة خطأ،
      // منجرب نعمل resync كامل بالبيانات المصفّاة الحالية (بدل تحديث جزئي) عشان أغلب الحالات
      // بتنحل لحالها من دون ما المستخدم يحتاج يبدّل فريم يدوياً
      console.error("chart data error:", err);
      try {
        const safe = sanitizeCandles(allCandles.slice(0, revealCount));
        seriesRef.current.setData(safe);
        chartRef.current?.timeScale().fitContent();
      } catch (err2) {
        console.error("chart resync failed:", err2);
