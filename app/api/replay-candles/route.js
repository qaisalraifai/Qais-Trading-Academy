...
/* ⚠️ **نزلت ٢٧ → ١٢ ثانية.** ٢٧ كانت منطقية لما كان الطلب هو المحاولة
   الوحيدة: نجرّب لآخر نفس قبل ما نتراجع. بس صار فوقها فواصل إعادة ومعها
   قراءة المخزن، فالمجموع تجاوز `maxDuration` (٣٠) وطلّع **504** بالإنتاج.

   وما عاد إلها داعي أصلاً: النافذة اللي بتنجح بتتخزّن بـ`candle_cache`
   فما بتنطلب مرتين، واللي بتفشل بيعيدها العميل بالخلفية بتراجع تصاعدي.
   فالطلب الطويل ما عاد يشتري إشي — بس بيقرّب من السقف. */
const DUKASCOPY_TIMEOUT_MS_ANCHOR = 12000;
...
export async function GET(req) {
  ...
  if (dukSymbol) {
    ...
    const dukResult = await withTimeout(
      /* ⚠️ الميزانية بتنمرّر عشان الفاصل بين محاولات التقليص يوقف قبل المهلة
         بدل ما ياكلها ويسقط الطلب كله لمزوّد أضعف. */
    ...
    /* ══════════════════════════════════════════════════════════════════
       🔴 **بوضع القص: Dukascopy وحده — بلا أي تراجع.**
       ------------------------------------------------------------------
       بقراره (٢٠٢٦-٠٨-٢٨): «معلومات القص كلها خليها بس من Dukascopy».

       وهو الصح تقنياً: المزوّدون التانيون **ما بيوصلوا** لتاريخ القص أصلاً
       على الفريمات اللحظية — يوهو محدود ~٥٨ يوم لربع الساعة و~٢٩ للدقيقة،
       وTwelve Data قريب منه. فلما نتراجع إلهم بقص من ٢٠٠٦، بيرجّعوا شموع
       **٢٠٢٦** بنجاح ظاهري، والواجهة بتلاقي إنه ما في ولا شمعة قبل نقطة
       القص فبتطلّع «تم البدء من أقرب نقطة متاحة» وبتحطّ الشارت بمكان غلط.

       يعني التراجع هون ما بينقذ شي — بيحوّل فشلاً واضحاً لبيانات مضلّلة.
       الخطأ الصريح أنضف: بيقول السبب، وما بيوهم إنه القص اشتغل.

       ⚠️ الحد **بوضع القص وحده**. المباشر بيضل يتراجع زي ما كان — هناك
