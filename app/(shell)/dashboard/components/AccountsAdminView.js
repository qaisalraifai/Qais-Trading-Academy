...
import { gold, s, glass, transition, gradientGold, shadowGold, displayStack, monoStack } from "@/app/admin/styles";
import StatCard from "@/app/admin/components/StatCard";
import Toolbar from "@/app/admin/components/Toolbar";
import FilterChips from "@/app/admin/components/FilterChips";
import KpiRow from "@/app/admin/components/KpiRow";
import { SubscriptionsTrendChart, RevenueBarChart } from "@/app/admin/components/Charts";
import UsersTable from "@/app/admin/components/UsersTable";
import UserDrawer from "@/app/admin/components/UserDrawer";
import ActivityFeed from "@/app/admin/components/ActivityFeed";
import QuickActions from "@/app/admin/components/QuickActions";
import AffiliatesPanel from "@/app/admin/components/AffiliatesPanel";
...
export default function AccountsAdminView({ username }) {
 ...
 <StatCard icon={Users} label="إجمالي المستخدمين" value={cards?.totalUsers ?? 0} color={gold} sparkline={trend} sub="آخر 30 يوم" delta={usersDelta} />
 <StatCard icon={Activity} label="نشطون الآن" value={cards?.activeNow ?? 0} color="#10E5A0" sub="آخر 15 دقيقة" />
 <StatCard icon={Crown} label="أعضاء VIP" value={cards?.vipCount ?? 0} color="#DCD4F7" />
