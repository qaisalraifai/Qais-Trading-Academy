...
export default function ReplayClient({ userId, paneId = "main", isPrimary = true, initialAsset = null, fillContainer = false, chromeSlots = null, chromeActive = true, onRequestFullscreen = null }) {
 ...
 لمّا الطالبة تقص وتحلل وترجع للمباشر ()، بنلقط"لقطة" كاملة من حالة
  ...
  /* ═══════════════════════════════════════════════════════════════════════════
     تعميق تدريجي — **طلبات صغيرة متتالية بدل طلب ضخم واحد**.
     ---------------------------------------------------------------------------
     🔴 المشكلة: طلب واحد بـ`count=20000` على اليومي بيطلب **٢٣ سنة أرشيف**،
     وDukascopy بترفضه فبينزل التقليص للربع — يعني ٢٠٢٠ بدل ٢٠٠٣. وحاولت
     أعالجها بتوسيع المهلة والتهدئة والسقوف، وكلها فشلت أو طلّعت ٥٠٠.

     والقياس الموثّق بالمشروع كان قدّامي من البداية:

         count=5000 → 429          count=1500 → **نجح** (0.4s)
         count=3000 → 429          count=900  → **نجح** (0.3s)

     يعني الطلبات الصغيرة بتنجح **دايماً**. فالعمق مش مسألة إقناع المزوّد
     بطلب ضخم — هو مسألة **تكرار طلبات صغيرة ودمجها**.

     المكسب بالجولة الواحدة (`count=1500`):

         يومي ٥.١ سنة · ٤ ساعات ٣١٣ يوم · ساعة ٧٨ يوم · ١٥ دقيقة ٢٠ يوم

     أربع جولات = اليومي **٢٠.٥ سنة**.

     ⚠️ بتشتغل بالخلفية بعد أول رسم — ما بتأخّر العرض ولا بتزاحم المستخدم.
     ⚠️ وبتوقف أول ما تبطّل تربح شموع: مصدر ما عنده أعمق ما بيتحسّن بالإلحاح.
     ⚠️ **و`revealCount` فهرس مش وقت** — فإضافة تاريخ أقدم بتزحزح الفهارس،
     ولازم يتعدّل بعدد اللي انضاف قبله وإلا نقطة القص بتقفز.
     ═══════════════════════════════════════════════════════════════════════════ */
  ...
  useEffect(() => {
    ...
    const DEEPEN_COUNT = 1500;   // مقيس: بينجح دايماً
    const BUFFER_BARS = 300;     // مطابق لـ`bufferSeconds` بالخادم
    const ROUNDS = 4;
    const GAP_MS = 3500;         // أطول بوضوح من نافذة حد الأرشيف
    const MIN_GAIN = 30;
