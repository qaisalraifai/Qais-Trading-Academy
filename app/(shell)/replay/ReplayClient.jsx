...
export default function ReplayClient({ userId }) {
 ...
 لمّا الطالبة تقص وتحلل وترجع للمباشر ()، بنلقط"لقطة" كاملة من حالة
  ...
  useEffect(() => {
    ...
    /* ⚠️ الحدّ الأدنى **٥ ثواني مش ١.٢**. القياس بيقول ~٤٠٠ملّي بتفشل و٦–١٥
       ثانية بتمرّق باستمرار — و١.٢ أقرب لمنطقة الفشل منها للنجاح. وكل جولة
       مرفوضة بتحرق النافذة اللي بتحتاجها اللي بعدها، فالسرعة الوهمية بتطلع
       أبطأ فعلياً. ٥ ثواني على حافة المنطقة الآمنة، وبتتسارع بس ما بتنزل
       تحتها. */
    const GAP_MIN = 5000;
    const GAP_MAX = 30000;
    let gap = GAP_MIN;
