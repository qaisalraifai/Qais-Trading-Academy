  ...
  return (
    <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 0, padding: "1.8rem", marginBottom: "1.5rem" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.5rem", flexWrap: "wrap", gap: "0.8rem" }}>
        ...
        <a href="/mlm" style={{ color: GOLD, fontSize: "0.85rem", textDecoration: "none", border: `1px solid #3E5478`, borderRadius: 3, padding: "0.45rem 1rem" }}>
          {t("affiliate.treeViewLink")}
        </a>
      </div>

      {/* شرح الشجرة الثنائية ببساطة */}
      <div style={{ background: "#111726", borderRadius: 0, padding: "1.2rem 1.4rem", marginBottom: "1.5rem" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "0.8rem", marginBottom: "1.6rem" }}>
        {BONUS_ITEMS.map((b) => (
          <div key={b.title} style={{ background: "#111726", border: `1px solid ${BORDER}`, borderRadius: 3, padding: "0.9rem 1rem" }}>
            <div style={{ fontSize: "1.1rem", marginBottom: 4 }}>{b.icon} <span style={{ fontWeight: 700, fontSize: "0.85rem" }}>{b.title}</span></div>
            <div style={{ fontSize: "0.75rem", color: "#5D6880", lineHeight: 1.6 }}>{b.desc}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "flex", gap: "0.6rem", overflowX: "auto", paddingBottom: 4 }}>
        {RANKS.map((r, i) => (
          <div key={r.name} style={{ display: "flex", alignItems: "center", gap: "0.6rem", flexShrink: 0 }}>
            <div style={{ background: "#111726", border: `1px solid #3E5478`, borderRadius: 3, padding: "0.7rem 1rem", textAlign: "center", minWidth: 130 }}>
              <div style={{ fontWeight: 700, color: GOLD, fontSize: "0.85rem" }}>{r.name}</div>
              <div style={{ fontSize: "0.7rem", color: "#5D6880", marginTop: 2 }}>{r.req}</div>
            </div>
