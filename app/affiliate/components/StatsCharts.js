  ...
  return (
    <section id="stats" style={{ scrollMarginTop: 90, marginBottom: "1.4rem" }}>
      <div style={card} className="qta-animate-in">
        ...
        <p style={{ color: "#9A9A9A", fontSize: "0.82rem", marginBottom: "1.2rem" }}>تابع أداء رابطك وأرباحك على مر الوقت.</p>

        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: "0.8rem", marginBottom: "1.4rem" }}>
          {funnelStats.map((st) => (
            <div key={st.label} style={{ background: "rgba(255,255,255,0.02)", border: `1px solid ${BORDER}`, borderRadius: 12, padding: "1rem", textAlign: "center" }}>
              <p style={{ color: "#9A9A9A", fontSize: "0.72rem", marginBottom: 6 }}>{st.label}</p>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "0.6rem", marginBottom: "0.9rem" }}>
          <p style={{ fontWeight: 700, fontSize: "0.88rem", color: "#EAECEF" }}>
            الأرباح — إجمالي الفترة: <span style={{ color: GOLD, fontFamily: monoStack }}>${fmt(totalForPeriod)}</span>
          </p>
          <div style={{ display: "flex", gap: 6, background: "rgba(255,255,255,0.02)", border: `1px solid ${BORDER}`, borderRadius: 10, padding: 4 }}>
            {PERIODS.map((p) => (
              <button
                key={p.key}
                ...
                style={{
                  ...
                  color: period === p.key ? "#16130a" : "#9A9A9A",
                  fontWeight: 700,
                  fontSize: "0.75rem",
