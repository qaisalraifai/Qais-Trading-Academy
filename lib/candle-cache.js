...
/* ⚠️ **الاسم فيه رقم إصدار — ارفعه لما تصير البيانات المخزَّنة مشكوكاً فيها.**
   ---------------------------------------------------------------------------
   صار فعلياً (٢٠٢٦-٠٨-٢٥): عطل بدمج الفريمات كان يخلط شموع فريم بفريم تاني
   ويخزّنها. العطل انصلح، بس **الإصلاح منع التلوّث الجديد وما نضّف القديم** —
   فمتصفّحات المستخدمين ضلّت ترسم من كاش متلوّث. المستخدم شاف ناسداك على
   ١٣٠٠٠ والسعر الحيّ ٢٩٠٨٩، ونافذة التصفّح المخفي أثبتت السبب.

   🔴 الدرس: **الكاش بلا إصدار = عطل بيانات دائم**. إصلاح الكود ما بيلمس اللي
   انخزّن قبله، والمستخدم ما بيعرف إنه في كاش أصلاً عشان يمسحه.

   رفع الاسم بيخلّي كل متصفّح يفتح قاعدة جديدة فارغة، والقديمة بتضل مهجورة
   (المتصفّح بينضّفها مع ضغط التخزين). سطر واحد، وبيشفي كل المستخدمين تلقائياً. */
/* ⚠️ v3: المخزون بـv2 اتبنى بدمج **بالطابع الخام**، فاليومي فيه شمعتان
   لليوم الواحد عند حدود التوقيت الصيفي (مقيس: barsPerDay 1.6). الإصلاح
   بالدمج ما بينظّف المحفوظ — فلازم يُبطَل. */
const DB_NAME = "qta_candles_v3";
const STORE = "series";
const DB_VERSION = 1;

/* ⚠️ أسماء القواعد القديمة — بتنحذف صراحةً بدل ما نستنى المتصفّح. */
const LEGACY_DB_NAMES = ["qta_candles_v1", "qta_candles_v2"];
...
/** بيحذف قواعد الإصدارات القديمة. ما بيرمي ولا بيعطّل شي لو فشل. */
function dropLegacyDbs() {
  for (const name of LEGACY_DB_NAMES) {
    try { indexedDB.deleteDatabase(name); } catch { /* بلا أثر */ }
  }
}
 ...
 */
/* ═══════════════════════════════════════════════════════════════════════════
   🔴 **الدمج كان بالطابع الخام — فاليوم الواحد بيصير شمعتين.**
   ---------------------------------------------------------------------------
   الشمعة اليومية بتنختم بمنتصف ليل **السوق** لا UTC، فطابعها بينزاح ساعة مع
   تبديل التوقيت الصيفي: `04:00` صيفاً و`05:00` شتاءً لنفس المؤشر. ولما ينلمّ
   المحفوظ مع الطازج (وكل واحد منهن انجلب بموسم مختلف)، الطابعان مختلفان
   فبينحفظوا الاتنين — **ونفس اليوم بيصير شمعتين**.

   مقيس على الإنتاج (٢٠٢٦-٠٨-٢٧) على NAS100 يومي:

       bars 3974 · days 2486 · barsPerDay **1.6** · dupes 0 · unsorted 0

   `dupes: 0` لأنه بيقارن الطوابع تماماً — فالخلل مرق من كل الفحوصات.

   الحل: المفتاح **بُعد الشمعة** مش الطابع. لليومي = رقم اليوم، وللباقي =
   الطابع كما هو (الفريمات اللحظية ما فيها هالانزياح).

   ⚠️ والطازج بيربح عند التعارض — هو الأحدث من المزوّد.
   ⚠️ وبنحتفظ بطابع **الطازج** كمان، عشان ما يضل نصف السلسلة على توقيت قديم.
   ═══════════════════════════════════════════════════════════════════════════ */
const DAY_SECONDS = 86400;

export function mergeCandles(oldCandles, freshCandles, intervalSeconds = 0) {
  if (!oldCandles?.length) return freshCandles || [];
  if (!freshCandles?.length) return oldCandles;
  /* اليومي وأكبر: نفس اليوم = نفس الشمعة مهما كانت الساعة. */
  const byDay = intervalSeconds >= DAY_SECONDS;
  const keyOf = (c) => (byDay ? Math.floor(c.time / DAY_SECONDS) : c.time);
  const byKey = new Map();
  for (const c of oldCandles) byKey.set(keyOf(c), c);
  for (const c of freshCandles) byKey.set(keyOf(c), c);
