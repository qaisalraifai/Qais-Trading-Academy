...
test("🔴 الطلب ما بينتظر جوّاه أبداً — الانتظار كان يطلّع 504", async () => {
  /* الفواصل بين المحاولات صحيحة كفكرة (مقيس: ~٤٠٠ملّي بتفشل · ٦–١٥ ثانية
     بتمرّق) بس **مكانها غلط**: مهلة القص ٢٧ + تلات فواصل ٦ = ١٨ + الجلبات
     + قراءة المخزن… و`maxDuration` ٣٠. فالطلب صار يتجاوز السقف بنيوياً
     وVercel بترجّع 504 — وهاد ظهر بالإنتاج فعلاً.

     الانتظار انتقل للعميل: بالخلفية، بلا سقف، وبتراجع تصاعدي. */
  const s = spy({ failTimes: 2 });
  const t0 = Date.now();
  await fetchDukascopyCandles("xauusd", "15min", 3000, CUT_2006, 27000, { getRates: s });
  const elapsed = Date.now() - t0;

  assert.equal(s.calls, 3, "التقليص بيضل شغّال — بس بلا نوم بينهن");
  assert.ok(elapsed < 1000, `ممنوع أي انتظار جوّا الطلب — طلع ${elapsed}ms`);
});

test("⚠️ وأقصى مسار بيضل بعيداً عن سقف الدالة", async () => {
  /* الحارس الحقيقي: مهما فشلت المحاولات، الدالة ما بتقترب من ٣٠ ثانية.
     المهلة الصارمة (`withTimeout` بالمسار) هي السقف، وهاد بيثبت إنّ الكود
     نفسه ما بيضيف إليها إشي. */
  const s = spy({ failTimes: 2 });
  const t0 = Date.now();
  await fetchDukascopyCandles("xauusd", "15min", 3000, CUT_2006, 27000, { getRates: s });
  assert.ok(Date.now() - t0 < 3000, `المسار الكامل لازم يخلص بثواني — طلع ${Date.now() - t0}ms`);
});

test("والمسار المباشر كمان بيفشل بسرعة", async () => {
  const s = spy({ failTimes: 2 });
  const t0 = Date.now();
