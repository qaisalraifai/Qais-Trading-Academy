...
/* ══════════ 🔴 الانزياح المقيس على الإنتاج (٢٠٢٦-٠٨-٢٥) ══════════ */

/** موقع الفهرس كنسبة من عرض النافذة — هاد اللي بيحدّد البكسل فعلاً. */
const frac = (idx, r) => (idx - r.from) / (r.to - r.from);

/** سلسلة أسبوعها `perWeek` شمعة، بتنتهي عند نفس اللحظة. */
function series(perWeek, count) {
  const times = [];
  for (let w = 0; times.length < count; w++)
    for (let d = 0; d < perWeek && times.length < count; d++)
      times.push((w * 7 + d) * DAY);
  const end = times[times.length - 1];
  return times.map((t) => t - end); // كلهن بينتهوا عند 0
}

test("🔴 آخر شمعة بتتحاذى رغم اختلاف كثافة الشموع", () => {
  /* الأرقام من `__qtaPaneInfo()` على الإنتاج:
       main    3520 شمعة · logical 3438.0643 → 3568.8710
       compare 2512 شمعة
     الحافة اليمنى واقعة 49.87 شمعة **بعد** آخر شمعة (منطقة rightOffset).
     ناسداك ~298 شمعة/سنة (فيها الأحد) وSP500 ~251 — فبنفس الفترة الأساسي
     بيحشر شموعاً أكتر، وآخر شمعة كانت بتنزاح ~5.7% من العرض. */
  const main = series(6, 3520);   // فيها شمعة الأحد
  const cmp = series(5, 2512);    // بلا أحد
  const r = { from: 3438.0643217054267, to: 3568.871007751938 };

  const out = mapLogicalRange(main, cmp, r);
  assert.ok(out, "لازم يترجم");

  const fMain = frac(main.length - 1, r);
  const fCmp = frac(cmp.length - 1, out);
  assert.ok(
    Math.abs(fMain - fCmp) < 0.005,
    `آخر شمعة لسا منزاحة: الأساسي ${(fMain * 100).toFixed(2)}% مقابل ${(fCmp * 100).toFixed(2)}%`
  );
});

test("⚠️ بتثبيت الحافة الفاضية كان الانزياح ~5.7% — إثبات إنّ المرساة هي الفرق", () => {
  /* نفس المدخلات بالسلوك القديم (تثبيت الحافتين) — لازم ينزاح فعلياً، وإلا
     الاختبار اللي فوق بيمرق على أي تنفيذ. */
  const main = series(6, 3520);
  const cmp = series(5, 2512);
  const r = { from: 3438.0643217054267, to: 3568.871007751938 };

  const si = medianInterval(main);
  const di = medianInterval(cmp);
  const old = {
    from: logicalAtTime(cmp, timeAtLogical(main, r.from, si), di),
    to: logicalAtTime(cmp, timeAtLogical(main, r.to, si), di),
  };
  const drift = Math.abs(frac(main.length - 1, r) - frac(cmp.length - 1, old));
  assert.ok(drift > 0.03, `القديم كان لازم ينزاح بوضوح، وطلع ${(drift * 100).toFixed(2)}%`);
});

test("نافذة كلها جوّا البيانات → نفس السلوك السابق بالضبط", () => {
  const main = series(6, 500);
  const cmp = series(5, 400);
  const r = { from: 300, to: 420 };
  const si = medianInterval(main);
  const di = medianInterval(cmp);

  const out = mapLogicalRange(main, cmp, r);
  assert.ok(Math.abs(out.from - logicalAtTime(cmp, timeAtLogical(main, r.from, si), di)) < 1e-9);
  assert.ok(Math.abs(out.to - logicalAtTime(cmp, timeAtLogical(main, r.to, si), di)) < 1e-9);
});

test("🔴 الحلقة بتنتهي كمان مع منطقة فراغ على الطرفين", () => {
  const main = series(6, 900);
  const cmp = series(5, 700);
  let a = { from: 830, to: 949 }; // ~50 شمعة فراغ بعد آخر وحدة
  let b = null;
  let applies = 0;

  for (let i = 0; i < 500; i++) {
    const tb = mapLogicalRange(main, cmp, a);
    if (tb && shouldApplyRange(b, tb, 0.25)) { b = tb; applies++; } else break;
    const ta = mapLogicalRange(cmp, main, b);
    if (ta && shouldApplyRange(a, ta, 0.25)) { a = ta; applies++; } else break;
  }
  assert.ok(applies < 10, `الحلقة ما وقفت — ${applies} تطبيق`);
  assert.ok(a.to - a.from > 100 && a.to - a.from < 140, `عرض النافذة انفلت: ${a.to - a.from}`);
});

test("نافذة كلها بالفراغ بترجّع null بدل مدى عبثي", () => {
  const main = series(6, 100);
  const cmp = series(5, 80);
  assert.equal(mapLogicalRange(main, cmp, { from: 200, to: 300 }), null);
});
