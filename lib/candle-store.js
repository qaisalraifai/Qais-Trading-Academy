/* ════════════════════════════════════════════════════════════════════════
   ...
   ═══ الدلاء ═══
   الدلو = `intervalSec × 1440` ثانية، فكل دلو ~١٤٤٠ شمعة مهما كان الفريم:

       دقيقة   يوم واحد      ساعة     ٦٠ يوم
       ٥ دقايق ٥ أيام        ٤ ساعات  ٢٤٠ يوم
       ١٥ دقيقة ١٥ يوم       يومي     ~٤ سنين

   وهاد مقصود من وجهين: حجم الصف ثابت تقريباً (~١٠٠ ك.ب)، ومدى الدلو
   للفريمات اللحظية بيضل **تحت عتبة الأرشيف المقيسة** (~٣٦٠ يوم) — فأي
   دلو ناقص بينجلب بطلب واحد بينجح.
...
export async function readRange(symbol, interval, intervalSec, fromTs, toTs) {
  ...
  const empty = { candles: [], have: new Set(), want };
  ...
  try {
    ...
    return { candles: all.filter((c) => c.time >= fromTs && c.time <= toTs), have, want };
  } catch {
