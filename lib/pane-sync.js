...
export function mapLogicalRange(srcTimes, dstTimes, range) {
  ...
  const srcLast = srcTimes.length - 1;

  /* ═══════════════════════════════════════════════════════════════════════
     🔴 **المرساة لازم تكون على شمعة، مش على حافة فاضية.**
     ---------------------------------------------------------------------
     عندنا درجتا حرية بس (`from` و`to`)، يعني منقدر نثبّت **لحظتين** بالضبط
     وباقي النافذة بينتج عن البيانات.

     أول نسخة ثبّتت **حافتي النافذة**. وبالقياس الفعلي طلعت الحافة اليمنى
     واقعة **٤٩.٨٧ شمعة بعد آخر شمعة** (منطقة `rightOffset` الفاضية) — يعني
     درجة حرية كاملة انصرفت على فراغ، وآخر شمعة (أهم نقطة بصرياً) انزاحت.

     ليش الانزياح أصلاً: عدد الشموع بنفس الفترة بيختلف بين المزوّدين. مقيس
     على الإنتاج — ناسداك ٣٥٢٠ شمعة يومية منذ ٢٠١٤ (~٢٩٨/سنة، فيها شمعة
     الأحد من Dukascopy) مقابل SP500 ٢٥١٢ منذ ٢٠١٦ (~٢٥١/سنة، بلا أحد).
     فبنفس النافذة الأساسي بيحشر ٨١ شمعة والمقارنة ٦٤. والبكسل بينقسم على
     **الشمعة** مش على الوقت.

     الحل: المرساتان بتنقصّا لمدى بيانات المصدر، فبتصيرا على شمعتين
     حقيقيتين. لو النافذة كلها جوّا البيانات، هاد **بيرجع للسلوك السابق
     حرفياً** (f1=0 · f2=1).

     ⚠️ الثمن: منطقة الفراغ بتطلع بعرض مختلف بين اللوحتين. مقصود — الفراغ
     ما فيه شي يتحاذى.
     ═══════════════════════════════════════════════════════════════════════ */
  const a1 = Math.min(Math.max(range.from, 0), srcLast);
  const a2 = Math.min(Math.max(range.to, 0), srcLast);

  // نسبة موقع كل مرساة من عرض النافذة (0 = الحافة اليسرى · 1 = اليمنى)
  const f1 = (a1 - range.from) / width;
  const f2 = (a2 - range.from) / width;

  const d1 = logicalAtTime(dstTimes, timeAtLogical(srcTimes, a1, si), di);
  const d2 = logicalAtTime(dstTimes, timeAtLogical(srcTimes, a2, si), di);
  if (!Number.isFinite(d1) || !Number.isFinite(d2)) return null;

  /* المرساتان قريبتان لبعض (نافذة كلها فراغ تقريباً) → القسمة بتنفجر.
     منرجع null بدل ما نطلّع مدى عبثي. */
  if (f2 - f1 < 1e-6 || d2 - d1 <= 0) return null;

  const dstWidth = (d2 - d1) / (f2 - f1);
  const from = d1 - f1 * dstWidth;
  const to = from + dstWidth;
