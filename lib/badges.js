...
/**
 * يحسب إحصائيات المسوّق اللازمة لفحص الشارات (شبكة 3 مستويات + أرباح + نقرات).
 */
export async function computeAffiliateStats(admin, affiliateId) {
  const { data: level1 } = await admin.from("profiles").select("id").eq("referred_by", affiliateId);
  const level1Ids = (level1 || []).map((r) => r.id);

  let level2Ids = [];
  if (level1Ids.length > 0) {
    const { data } = await admin.from("profiles").select("id").in("referred_by", level1Ids);
    level2Ids = (data || []).map((r) => r.id);
  }
  let level3Count = 0;
  ...
  const { data: commissions } = await admin
    .from("commissions")
    .select("commission_amount, level, source_user_id")
    .eq("affiliate_id", affiliateId);
  const totalEarned = (commissions || []).reduce((sum, c) => sum + (Number(c.commission_amount) || 0), 0);
  const paidReferrals = new Set((commissions || []).filter((c) => c.level === 1).map((c) => c.source_user_id)).size;
  ...
  return {
    totalNetwork: level1Ids.length + level2Ids.length + level3Count,
    totalEarned,
    paidReferrals,
    clicks,
    conversions,
