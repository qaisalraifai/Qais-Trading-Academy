...
/* ============================================================================
   معرّف الجلسة **لو موجود** — بلا ما يرمي وبلا ما يرفض.

   ---------------------------------------------------------------------------
   لمسارات غياب الجلسة فيها حالة مشروعة مش خطأ. الحالة الوحيدة القائمة:
   `/api/create-profile` — بينندى بعد `signUp()` مباشرة، ووقتها ممكن ما يكون
   في جلسة أصلاً (لما تفعيل الإيميل مطلوب، `signUp` بترجّع مستخدماً بلا جلسة).

   فالمسار ما بيقدر **يشترط** جلسة… بس لما تكون موجودة بتصير **أقوى دليل**
   على إنّ المنادي صاحب الحساب. هاي الدالة بتخلّي هالاستعمال ممكناً:
   بترجّع المعرّف لو في جلسة سليمة، و`null` بأي حالة تانية (ما في كوكي،
   كوكي مكسور، توكن منتهي، خادم ما ردّ) — بلا ما ترمي ولا ترجّع رداً.

   ⚠️ `null` معناها **«ما قدرنا نتأكد»** مش «مش صاحبه». المنادي لازم يعامل
   الحالتين بشكل مختلف.
   ============================================================================ */
