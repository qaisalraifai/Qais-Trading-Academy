...
import { payFastStartBonus } from "@/lib/fast-start-engine";
import { getSetting } from "@/lib/mlm-settings";

// ثوابت CV فقط (الفصل 2) — 1 دينار = 1 CV، هاي مربوطة بقيمة الاشتراك نفسها
// (300/100) مش مبلغ عمولة، فمنطقي تضل ثابتة بالكود. أما مبالغ العمولات
// (Direct/Renewal) فبتُقرأ من mlm_settings (قابلة للتعديل من لوحة الأدمن).
...
export async function processMlmCommissionsForPayment(supabaseAdmin, { userId, paymentId, isFirstPayment }) {
  ...
  if (profile.sponsor_id) {
    if (isFirstPayment) {
      const directAmount = await getSetting(supabaseAdmin, "direct_bonus_amount");
      await insertCommissionAndPay(supabaseAdmin, {
        ...
        bonusType: "direct",
        amount: directAmount,
        notifyMessage: `حصلت على ${directAmount} دينار عمولة مباشرة (Direct Bonus)`,
      });
      // الفصل 5 بند 1: مكافأة الانطلاقة السريعة — للعضو الجديد نفسه، أول اشتراك فقط
      await payFastStartBonus(supabaseAdmin, userId).catch((e) =>
        console.error("payFastStartBonus failed:", e.message)
      );
      // مكافآت الإنجاز لراعي العضو (عدد مباشرينه زاد بواحد لتوّه)
      await checkAchievements(supabaseAdmin, profile.sponsor_id).catch((e) =>
        console.error("checkAchievements failed:", e.message)
      );
    } else {
      const renewalAmount = await getSetting(supabaseAdmin, "renewal_bonus_amount");
      await insertCommissionAndPay(supabaseAdmin, {
        beneficiaryId: profile.sponsor_id,
        sourceUserId: userId,
        paymentId,
        ...
        notifyMessage: `حصلت على ${renewalAmount} دينار عمولة تجديد (Renewal Bonus)`,
      });
    }
  }
  ...
  for (const ancestorId of touchedAncestors) {
    ...
    await checkAchievements(supabaseAdmin, ancestorId).catch((e) =>
      console.error("checkAchievements failed for", ancestorId, e.message)
    );
  }

  // TODO (مراحل جاية): Infinity Bonus شهري (انظر lib/infinity-engine.js عبر cron)
}
