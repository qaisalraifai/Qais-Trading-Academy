...
export async function fetchDukascopyCandles(instrument, interval = "15min", wanted = 1000, anchorTimestamp = null, budgetMs = 0) {
  ...
  /* ============================================================================
     ...
     ============================================================================ */
  /* ⚠️ **الاستكمال محكوم بالوقت المتبقّي مش بعدد قطع ثابت.**
     الراوت بيلفّ هالنداء بـ`withTimeout`: **٨ ثواني** بلا مرساة و٢٧ معها.
     عدد ثابت من القطع بيتجاوز الـ٨ ثواني، فيسقط الطلب كله ويتراجع لمزوّد
     أضعف — يعني «تحسين العمق» بينتهي **بعمق أقل** من قبله.
     منوقف عند ٧٠٪ من الميزانية عشان يضل وقت للتحويل والرد. */
  const deadline = budgetMs > 0 ? startedAt + budgetMs * 0.85 : 0;
  /* ⚠️ **كل خطوة بتنسجّل.** التقليص الصامت هو اللي خلّى العطل يعيش شهوراً:
     `usedFallback:false` و`errors:null` وهو بيسلّم ربع العمق. وأنا صرفت تلات
     شحنات أخمّن ليش الاستكمال ما اشتغل — لأنه ما كان في شي بيقول.
     بيطلع بـ`__qtaPaneInfo().compareSource.duk`. */
  const trace = {
    spanFactor: usedFrom === fromMs ? 1 : Number(((toMs - usedFrom) / (toMs - fromMs)).toFixed(3)),
    budgetMs,
    elapsedBeforeBackfillMs: Date.now() - startedAt,
    chunksTried: 0,
    chunksOk: 0,
    stopped: null,
  };

  /* تهدئة كافية تخرج من نافذة حد الأرشيف. القياس بيقول عنا ~٤.٤ ثانية فاضية
     بعد الطلب الأساسي، وقطعة وحدة ناجحة بتنقل العامل من 0.25 لـ0.5 — يعني
     نفس عمق الشارت الأساسي بالضبط (2014-10-29). فالأولوية لقطعة **تنجح**
     مش لعدة قطع تنرفض. */
  const BACKFILL_COOLDOWN_MS = 1400;
  const MAX_BACKFILL_CHUNKS = 8;
  if (usedFrom > fromMs) {
    const chunkSpan = toMs - usedFrom;
    let cursor = usedFrom;
    for (let i = 0; i < MAX_BACKFILL_CHUNKS && cursor > fromMs; i++) {
      if (deadline && Date.now() >= deadline) { trace.stopped = "الميزانية خلصت"; break; }
      const chunkTo = cursor;
      const chunkFrom = Math.max(fromMs, chunkTo - chunkSpan);
      if (chunkTo - chunkFrom < secPerBar * 1000 * 2) { trace.stopped = "وصلنا الأقدم"; break; }
      trace.chunksTried++;
      try {
        /* 🔴 **التهدئة كانت ٢٥٠ملّي — وهاد رجّع 429 على أول قطعة.**
           -------------------------------------------------------------
           مقيس على الإنتاج (٢٠٢٦-٠٨-٢٧) عبر `__qtaPaneInfo()`:

               spanFactor 0.25 · budgetMs 8000 · elapsed 2447
               chunksTried 1 · chunksOk 0
               stopped: "رفض: Request failed with status 429"

           يعني **الوقت ما كان المشكلة** — استعمل ٢.٤ من ٦.٨ ثانية متاحة.
           الطلب الناجح لسا كان «طازة» عند Dukascopy، فالقطعة اللي بعده
           بـ٢٥٠ملّي وقعت جوّا نفس نافذة الحد.

           وهاد بيطابق الموثَّق فوق: الحد مربوط **بعدد ملفات الأرشيف**
           والتهدئة القصيرة ما بتتفاداه. فالتهدئة صارت من الميزانية
           المتوفّرة فعلياً بدل رقم رمزي. */
        await new Promise((r) => setTimeout(r, BACKFILL_COOLDOWN_MS));
        const older = await getHistoricalRates({
          instrument,
          dates: { from: new Date(chunkFrom), to: new Date(chunkTo) },
          timeframe,
          priceType: "bid",
          format: "json",
          volumes: true,
        ...
        if (!Array.isArray(older) || !older.length) { trace.stopped = "قطعة فاضية"; break; }
        raw = older.concat(raw);
        trace.chunksOk++;
        cursor = chunkFrom;
      } catch (e) {
        const msg = String(e?.message || e);
        /* محاولة تانية بتهدئة أطول — بس للرفض (429) وبس لو الوقت بيسمح.
           أي خطأ تاني ما بيتحسّن بالانتظار. */
        const isRateLimit = /429|rate/i.test(msg);
        const canRetry = isRateLimit && (!deadline || Date.now() + 2600 < deadline);
        if (!canRetry) {
          trace.stopped = `رفض: ${msg.slice(0, 80)}`;
          break;
        }
        trace.retried = (trace.retried || 0) + 1;
        try {
          ...
          if (!Array.isArray(older) || !older.length) { trace.stopped = "قطعة فاضية بعد الإعادة"; break; }
          raw = older.concat(raw);
          trace.chunksOk++;
          cursor = chunkFrom;
        } catch (e2) {
          trace.stopped = `رفض بعد الإعادة: ${String(e2?.message || e2).slice(0, 60)}`;
          break;
        }
      }
    }
    if (!trace.stopped && trace.chunksTried >= MAX_BACKFILL_CHUNKS) trace.stopped = "حد القطع";
  }
