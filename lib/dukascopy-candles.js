...
/* عامل المدى اللي نجح لكل (رمز+فريم). **بمستوى الوحدة عمداً** — جوّا الدالة
   بيتفضّى كل نداء فما بيوفّر شي. بيعيش بعمر العملية وبيفضى بأي بداية باردة. */
...
export async function fetchDukascopyCandles(instrument, interval = "15min", wanted = 1000, anchorTimestamp = null, budgetMs = 0) {
  ...
  /* ═══════════════════════════════════════════════════════════════════════
     🔴 **المحاولات الفاشلة كانت تاكل الميزانية كلها فما يضل وقت للاستكمال.**
     ---------------------------------------------------------------------
     المهلة بوضع اللايف **٨ ثواني** (`DUKASCOPY_TIMEOUT_MS_LIVE`) — وهاد قرار
     موثّق بالراوت: «الشارت يفتح بسرعة معقولة دايماً». فما بينمدّد.

     بس كل رفض بياخد ١.٢–١.٦ ثانية (مقيس)، فرمز زي `usa500idxusd` اليومي
     بيصرف ~٣ ثواني على العاملين ١ و٠.٥ **وهو عارف إنهن بينرفضوا**، وبعدها
     ~٢ ثانية على ٠.٢٥ الناجح. يعني الميزانية خلصت والاستكمال ما اشتغل ولا
     قطعة — وهاد ليش شحنة `1baabd3` ما غيّرت شي.

     الحل: نتذكّر العامل اللي نجح لكل (رمز+فريم) ونبلّش منه. الوقت المحرَّر
     (~٣ ثواني) بيروح للاستكمال بدل ما ينحرق على رفض متوقَّع.

     ⚠️ الذاكرة **بالعملية الواحدة** وبتفضى مع كل بداية باردة — يعني أول طلب
     بيدفع كلفة الاكتشاف، واللي بعده بيستفيد. ما بتنحفظ ولا بتنتقل.
     ⚠️ ولو العامل المحفوظ فشل، منكمل للأصغر منه عادي.
     ═══════════════════════════════════════════════════════════════════════ */
  const memoKey = `${instrument}|${timeframe}`;
  const memoFactor = spanMemo.get(memoKey);
  const attempts = memoFactor ? SPAN_RETRIES.filter((f) => f <= memoFactor) : SPAN_RETRIES;
  ...
  for (const factor of attempts) {
    ...
    try {
      ...
      if (Array.isArray(raw) && raw.length) { usedFrom = attemptFrom; spanMemo.set(memoKey, factor); break; }
