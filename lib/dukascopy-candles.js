...
export async function fetchDukascopyCandles(instrument, interval = "15min", wanted = 1000, anchorTimestamp = null) {
  ...
  try {
    raw = await getHistoricalRates({
      ...
      useCache: false,
      /* ============================================================================
         الخنق حسب حجم الطلب — Dukascopy بترجّع **HTTP 429** مش مهلة.
         ----------------------------------------------------------------------------
         قياس فعلي (٢٠٢٦-٠٨-١٥) عبر التطبيق المنشور على الذهب:
             count=900   → dukascopy نجح  (0.8s للـ4h · 0.5s لليومي)
             count=3000  → تراجع لمزوّد أدنى
         والرفض بيجي خلال ١.٢–١.٦ ثانية — يعني **مش انتهاء مهلة**، رفض صريح.

         السبب: المدى بينحسب secPerBar × count × 1.25، فcount=3000 يومي =
         ~١٠ سنوات = عشرات ملفات الأرشيف. مع batchSize=20 وتوقّف ١٥٠ms،
         بتنطلق دفعات متلاحقة بتضرب حد المعدل.

         فالمشكلة **حجم الطلب مش الوقت المستهلك**. الطلبات الكبيرة (تحليل،
         مش شارت تفاعلي) بتاخد خنق أهدأ وصبر أطول بين المحاولات — الشارت
         التفاعلي بيضل على الإعدادات السريعة لأنها شغّالة فعلاً.
         ============================================================================ */
      ...(count > 1500
        ? { batchSize: 8, pauseBetweenBatchesMs: 500, retryCount: 3, pauseBetweenRetriesMs: 1500 }
        : { batchSize: 20, pauseBetweenBatchesMs: 150, retryCount: 2, pauseBetweenRetriesMs: 400 }),
    });
  } catch (e) {
