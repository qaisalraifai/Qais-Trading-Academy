...
export async function fetchDukascopyCandles(instrument, interval = "15min", wanted = 1000, anchorTimestamp = null, budgetMs = 0) {
  ...
  /* تهدئة كافية تخرج من نافذة حد الأرشيف. القياس بيقول عنا ~٤.٤ ثانية فاضية
     ...
     مش لعدة قطع تنرفض. */
  /* ═══════════════════════════════════════════════════════════════════════
     🔴 **استكمال العمق جوّا نفس الطلب انشال — انقاس مرتين وفشل.**
     ---------------------------------------------------------------------
     الفكرة كانت: القطعة اللي نجحت مقاس مثبَت، فمنكمّل الأقدم بقطع بنفس
     الحجم. وفشلت بالإنتاج مرتين، والقياس واضح:

         تهدئة 250ملّي  → chunksTried 1 · chunksOk 0 · 429
         تهدئة 1400ملّي → chunksTried 1 · chunksOk 0 · 429  (memoHit:true)

     والمرة التانية الوقت كان متوفّراً (elapsed 2393 من 6800). يعني Dukascopy
     بترفض أي طلب أرشيف تاني **بنفس الاستدعاء** مهما انتظرنا — الحد مربوط
     بعدد الملفات زي ما هو موثّق فوق، والتهدئة داخل الطلب ما بتخرج من نافذته.

     البديل الشغّال: طلب **منفصل** من المتصفّح بعد ثواني بـ`anchor` على أقدم
     شمعة عندنا (شوفي `deepenCompare` بـ`ReplayClient.jsx`). هيك بينوزّع
     حمل الأرشيف على استدعاءات مستقلة — وهاي نفس التقنية الموثّقة اللي نجحت
     للفريمات اللحظية (١٦ قطعة من ١٨).

     ⚠️ انشال بدل ما ينترك معطّلاً: كان بيكلّف تهدئة ١.٤ ثانية **وطلب فاشل**
     على كل فتح شارت، بلا أي مقابل.
     ═══════════════════════════════════════════════════════════════════════ */
