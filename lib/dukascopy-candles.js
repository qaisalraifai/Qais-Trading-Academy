...
export async function fetchDukascopyCandles(instrument, interval = "15min", wanted = 1000, anchorTimestamp = null, budgetMs = 0) {
  ...
  /* ═══════════════════════════════════════════════════════════════════════
     🔴 **سقف المدى — ٢٣ سنة أرشيف بطلب واحد بتطيح الدالة.**
     ---------------------------------------------------------------------
     الحدّ فوق بيمنع النزول قبل ٢٠٠٣، بس ما بيمنع طلب **٢٣ سنة دفعة وحدة**.
     ولما نجح العامل ١ فعلاً (`first: 2003-01-02`)، طلعت **500** بلا جسم
     JSON — وهاد توقيع الانهيار الموثّق بأول الملف: `getHistoricalRates`
     بتطيح الـprocess بنفاد ذاكرة، و`try/catch` **ما بتمسكها**.

     مقيس على الإنتاج (٢٠٢٦-٠٨-٢٧): خمس ٥٠٠ متتالية، و`loadSource: null`
     يعني التحميل فشل والمعروض من الكاش. وتقصير المهلة (٢٧→٢٠) ما حلّها —
     فهي مش تجاوز وقت، هي حجم.

     الأدلة على الحدّ الآمن، من قياسات هالجولة:
         العامل 0.25 (~٦ سنين)  → بينجح دايماً
         العامل 0.50 (~١٢ سنة)  → بينجح أحياناً
         العامل 1.00 (~٢٤ سنة)  → بيطيح الدالة

     فالسقف ١٢ سنة: أعمق بست سنين من اللي كان (٢٠٢٠)، وبيشيل صنف الانهيار
     كاملاً لأنه ما بنطلب المدى القاتل أبداً.

     ⚠️ والعمق ما بيتوقف هون: `mergeCandles` بتراكم عبر التحميلات، فكل قصّة
     بتضيف لتاريخها المحفوظ بدل ما تبلّش من الصفر.
     ═══════════════════════════════════════════════════════════════════════ */
  const MAX_SPAN_MS = 12 * 365.25 * 24 * 60 * 60 * 1000;
  if (toMs - fromMs > MAX_SPAN_MS) fromMs = toMs - MAX_SPAN_MS;
  ...
  /* ⚠️ **٦ ثواني، مش ٢.٥.** قياس على الإنتاج بمسار القص بعد تهدئة ٢.٥:

         spanFactor 0.25 · elapsedBeforeBackfillMs 7549 · budgetMs 27000

     يعني التهدئة نفعت (المزوّد صار dukascopy بدل التراجع ليوهو)، بس ٢.٥
     ثانية ما كفّت لفتح نافذة الحد عند العاملين الأوسع — فوقع على الربع.
     والميزانية مستعملة ٧.٥ من ٢٧: **١١ ثانية كانت فاضية**، ومنها ~٤ كانت
     تنحرق على استكمال ميت (انشال فوق).

     بـ٦ ثواني: المحاولة الأولى فوراً، والتانية عند ~٧.٤، والتالتة عند
     ~١٤.٨ — كلهن تحت السقف (١٨.٩). يعني كل محاولة بتاخد نافذة أنظف. */
  const RETRY_COOLDOWN_MS = 6000;
  let attemptIndex = 0;

  for (const factor of attempts) {
    if (attemptIndex++ > 0 && budgetMs >= 15000 && Date.now() - startedAt + RETRY_COOLDOWN_MS < budgetMs * 0.85) {
  ...
  /* تهدئة كافية تخرج من نافذة حد الأرشيف. القياس بيقول عنا ~٤.٤ ثانية فاضية
     ...
     مش لعدة قطع تنرفض. */
  /* ⚠️ **استكمال العمق جوّا الطلب انشال — انقاس فاشلاً أربع مرات.**
     تهدئة ٢٥٠ملّي → 429 · ١٤٠٠ملّي → 429 · مع إعادة ٢٢٠٠ملّي → 429 ·
     وبميزانية ٢٧ ثانية → 429 كمان (`chunksTried 1 · chunksOk 0`).
     Dukascopy بترفض أي طلب أرشيف **تاني بنفس الاستدعاء** مهما انتظرنا.
     ⚠️ وكان بيصرف ~٤ ثواني وطلبين فاشلين — الوقت انتحوّل لمحاولات المدى
     اللي **بتنجح** فعلاً. */
