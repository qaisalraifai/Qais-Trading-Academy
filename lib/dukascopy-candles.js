...
export async function fetchDukascopyCandles(instrument, interval = "15min", wanted = 1000, anchorTimestamp = null, budgetMs = 0) {
  ...
  /* ⚠️ **استكمال العمق جوّا الطلب انشال — تلات قياسات وكلها فشلت.**
     تهدئة 250ملّي → 429 · تهدئة 1400ملّي → 429 · مع إعادة 2200ملّي → 429.
     والمرة الأخيرة بميزانية ٢٧ ثانية (`retried:1`)، فالوقت مش القيد:
     Dukascopy بترفض أي طلب أرشيف تاني **بنفس الاستدعاء**.
     البديل الشغّال: `deepenCompare` بـ`ReplayClient.jsx` — طلبات منفصلة.
     ⚠️ انشال بدل ما ينترك: كان يصرف ~٣.٦ ثانية وطلبين فاشلين بلا مقابل. */
