...
export async function fetchTwelveDataCandles(symbol, interval = "15min", wanted = 1000, anchorTimestamp = null) {
  ...
  const candles = values
    ...
    // Twelve Data برجّع شمعة منفصلة للسبت والأحد للمعادن أحياناً (رغم إنه
    // السوق شبه مقفول عطلة الأسبوع) - الفتح≈الأعلى≈الأدنى≈الإغلاق بيها
    // تقريباً، فبتطلع كـ"شحطة رفيعة"/خط مسطّح فاضي بدل شمعة طبيعية. TradingView
    // والبروكرات ما بتعرض هيك شموع أصلاً (بتدمج عطلة الأسبوع بشمعة الإثنين).
    // فبنشيلها من *كل* الفريمات (بطلب صريح - مش بس اليومي متل قبل)، لأن
    // المعادن هون هي الوحيدة يلي بتستخدم Twelve Data بهالمشروع أصلاً (مافي
    // كريبتو أو أسهم عبره) فما في خطر نشيل بيانات حقيقية بغلط.
    .filter((c) => {
