...
test("محتوى الأعضاء بيتطلب اشتراكاً", () => {
  for (const p of ["/dashboard", "/replay", "/trading-radar", "/reports", "/ai-trades", "/accounts"]) {
...
test("⚠️ حارس: كل صفحة بمنطقة الأعضاء مصنَّفة — ولا وحدة عامة بالغلط", () => {
  /* `/settings` الاستثناء الوحيد المقصود: إدارة الحساب لازم توصلها حتى لو
     الاشتراك واقف. أي صفحة تانية بتطلع «عامة» معناها إنها انضافت بلا ما
     تنصنّف — وهاد بيكشف محتوى مدفوع. */
  const INTENTIONAL_SESSION_ONLY = ["/settings"];
