 ...
 */
export function scanLegOrderBlocks(candles, window, fvgs) {
  ...
  for (let i = fromIndex + 1; i <= toIndex && i < candles.length; i++) {
    ...
    const invalid = moveDirUp ? lastPrice < invalidationLevel : lastPrice > invalidationLevel;
    ...
    if (status === "Invalid") continue; // بنحتفظ بالصالحة بس — هاي القائمة يلي بتشكّل "الذاكرة"
 ...
 */
export function scanAllLegsOrderBlocks(candles, legs, fvgs) {
  if (!Array.isArray(legs) || !legs.length) return [];
  const all = [];
  const seen = new Set();
  for (const leg of legs) {
    const window = { fromIndex: leg.fromIndex, toIndex: leg.toIndex, direction: leg.direction };
    const obs = scanLegOrderBlocks(candles, window, fvgs);
    for (const ob of obs) {
