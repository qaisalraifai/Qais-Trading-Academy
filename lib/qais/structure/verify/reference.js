...
export function validateReference(reference, fixture) {
  ...
  events.forEach((e, i) => {
    ...
    if (!Number.isFinite(e.price)) errors.push(`${at}: سعر غير رقمي`);
  ...
  return { ok: errors.length === 0, errors, warnings, eventCount: events.length, swingCount: swings.length };
}
...
export function scoreEvents(referenceEvents, engineEvents, candles, options = {}) {
  ...
  for (const ref of refs) {
    ...
    matches.push({
      ...
      priceDiff: Number.isFinite(ref.price) && Number.isFinite(e.price) ? +(e.price - ref.price).toFixed(5) : null,
      pricePct:
        Number.isFinite(ref.price) && Number.isFinite(e.price) && ref.price !== 0
          ? +((Math.abs(e.price - ref.price) / Math.abs(ref.price)) * 100).toFixed(4)
          : null,
    });
  }
