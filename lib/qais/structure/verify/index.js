...
function replayMetrics(replay) {
  ...
  const byType = {};
  for (const l of replay.latencies) {
  ...
  return {
    ...
    /* لو BOS/CHOCH طلعوا بتأخير > 0، معناها في اعتماد على المستقبل
       ما انتبهنا إله — أو العكس، تأخير سالب = نظر للمستقبل. */
    causalityViolations: replay.latencies.filter((l) => l.latencyCandles < 0).length,
  };
}
