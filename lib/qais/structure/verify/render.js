...
export function renderReport(r) {
  ...
  for (const [tf, d] of Object.entries(r.perTF)) {
    ...
    L.push(`  events   : BOS ${c.bos} · MSS ${c.mss} · كسر كاذب ${c.fakeBreaks} · كسر بالذيل ${c.wickBreaks}`);
  ...
  for (const [tf, d] of Object.entries(r.perTF)) {
    ...
    const rows = [
      ["تسطّح شموع متلاصقة (قمم)", e.adjacentFlatHighs.count],
      ["تسطّح شموع متلاصقة (قيعان)", e.adjacentFlatLows.count],
      ["بيفوتات ضاعت بالتعادل", e.pivotsSuppressedByExactTie.count],
      ["نوبات تجاوز بالذيل", e.wickBreaks.count + " (خام " + e.wickBreaksRaw.count + ")"],
