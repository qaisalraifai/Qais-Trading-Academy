...
/* ============================================================================
   ...
   ============================================================================ */
/* ============================================================================
   ⚠️ الشموع العشوائية (gridCandles) كانت تولّد **حدث واحد وصفر MSS**، فادعاء
   «صفر أحداث غير مستقرة» كان مبني على عيّنة من حدث واحد. ولازم مسار مشكّل
   عمداً حتى يطلع عدة BOS وعدة MSS.
   ============================================================================ */
function shapedPath(points, barsPerLeg = 12, seed = 7) {
  const rng = makeRng(seed);
  const out = [];
  let t = 1700000000;
  const H4 = 14400;
  const push = (o, h, l, c) => out.push({ time: (t += H4), open: o, high: h, low: l, close: c });

  for (let p = 0; p < points.length - 1; p++) {
    const a = points[p], b = points[p + 1], up = b > a;
    for (let i = 0; i < barsPerLeg; i++) {
      const o = a + ((b - a) * i) / barsPerLeg;
      const c = a + ((b - a) * (i + 1)) / barsPerLeg;
      const j = 0.15 + rng() * 0.6;
      push(o, Math.max(o, c) + j, Math.min(o, c) - j, c);
    }
    const nx = points[p + 2];
    if (nx != null && ((up && nx < b) || (!up && nx > b))) {
      const o = b - (up ? 0.3 : -0.3);
      push(o, up ? b + 2 : o + 0.6, up ? o - 0.6 : b - 2, up ? b - 0.25 : b + 0.25);
    }
  }
  return out;
}

const SHAPED = [100, 160, 130, 190, 120, 165, 95, 140, 70];

test("ما في خرق سببية ولا أحداث غير مستقرة (على عيّنة فيها BOS وMSS)", () => {
  const c = shapedPath(SHAPED);
  const r = replayIncremental(c, { timeframe: "h4", maxCandles: 400 });

  assert.equal(r.ok, true);
  const byType = r.latencies.reduce((a, l) => ((a[l.type] = (a[l.type] || 0) + 1), a), {});
  assert.ok(r.latencies.length >= 4, `عيّنة ضعيفة: ${r.latencies.length} حدث بس`);
  assert.ok((byType.MSS || 0) >= 1, `ما في MSS بالعيّنة — ${JSON.stringify(byType)}`);
  assert.ok((byType.BOS || 0) >= 1, `ما في BOS بالعيّنة — ${JSON.stringify(byType)}`);

  assert.equal(r.disappeared.length, 0, `${r.disappeared.length} حدث ظهر ثم اختفى`);
});

test("كل الأنواع بتنكتشف بلحظة الكسر — MSS مشمول", () => {
  /* ⚠️ النسخة السابقة كانت تفحص نوعاً واحداً وبتستثني MSS كلياً.
     وبالمنهجية الحالية الـMSS فوري متل الـBOS، فلازم نفس الشرط. */
  const c = shapedPath(SHAPED, 12, 91);
  const r = replayIncremental(c, { timeframe: "h4", maxCandles: 400 });

  const checked = { BOS: 0, MSS: 0 };
  for (const l of r.latencies) {
    /* الأحداث قبل نهاية الإحماء بينكتشفوا عند أول خطوة (warmup)، فتأخيرها
       أثر قياس مش سلوك محرك — بتنستثنى صراحةً. */
    if (l.eventIndex < 40) continue;
    assert.equal(l.latencyCandles, 0, `${l.type}@${l.eventIndex} تأخيره ${l.latencyCandles} شمعة`);
    checked[l.type] = (checked[l.type] || 0) + 1;
  }
  assert.ok(checked.BOS > 0 && checked.MSS > 0, `ما انفحص النوعين — ${JSON.stringify(checked)}`);
});
...
test("مطابقة ناجحة بتحسب الفارق الزمني والسعري بالتِك", () => {
  ...
  const events = [
    { id: "e1", type: "MSS", direction: "up", time: refTime + 14400 * 3, index: 103, price: 29812.5, strength: "Strong", confidence: 0.8, fakeBreak: false, reason: "x" },
  ];
  const m = matchTrade(
