...
const ev = (i, type, direction, price = 100) => ({ time: at(i), type, direction, price, reason: "سبب" });
...
test("تطابق تام = precision و recall = 1، وصفر أخطاء", () => {
  const refs = [ev(10, "BOS", "up"), ev(20, "CHOCH", "down"), ev(30, "MSS", "down")];
...
test("المحرك فوّت حدث = False Negative مؤكد", () => {
  const refs = [ev(10, "BOS", "up"), ev(30, "CHOCH", "down")];
...
test("نفس اللحظة بنوع مختلف = التباس تصنيف، مش True Positive", () => {
  ...
  assert.equal(s.details.typeConfusion[0].reference.type, "CHOCH");
});
...
test("الفارق الزمني والسعري بينحسبوا بإشارة صحيحة", () => {
  const refs = [{ time: at(20), type: "BOS", direction: "up", price: 100, reason: "x" }];
...
test("نوع غايب عن الطرفين = INSUFFICIENT_DATA مش دقة صفر", () => {
  ...
  assert.equal(s.byType.CHOCH.value, "INSUFFICIENT_DATA");
...
test("ترتيب إدخال التسميات ما بيغيّر النتيجة", () => {
  const refs = [ev(30, "MSS", "down"), ev(10, "BOS", "up"), ev(20, "CHOCH", "down")];
  const eng = [ev(20, "CHOCH", "down"), ev(30, "MSS", "down"), ev(10, "BOS", "up")];
