...
/* ============================================================================
   ٢.ج) المطابقة العالمية — حراس انحدار لعيبين أثبتتهم مراجعة مستقلة
   ============================================================================ */
test("تسمية مبكّرة ما بتبلع الحدث اللي بيخص تسمية لاحقة", () => {
  /* العيب: المطابقة الجشعة بترتيب الزمن كانت تخلّي BOS:up@10 يبلع
     MSS:down@12 (النوع والاتجاه غلط الاتنين) لأنه المرشّح الوحيد —
     فمحرك سليم تماماً كان يتقيّم recall 0 · precision 0. */
  const refs = [ev(10, "BOS", "up"), ev(12, "MSS", "down")];
  const eng = [ev(12, "MSS", "down")]; // مطابق تماماً للتسمية التانية
  const s = scoreEvents(refs, eng, candles, { windowCandles: 2 });

  assert.equal(s.truePositives, 1, "الحدث الصح لازم ينطابق مع تسميته");
  assert.equal(s.precision, 1, "ما في حدث محرك زايد");
  assert.equal(s.recall, 0.5, "تسمية وحدة من اتنين انلقطت");
  assert.equal(s.directionErrors, 0, "ما لازم تنبني مطابقة نوعها واتجاهها غلط الاتنين");
  assert.equal(s.details.matches[0].reference.type, "MSS");
});

test("النتيجة ما بتعتمد على إزاحة التسميات بالزمن", () => {
  const eng = (i) => [ev(i + 2, "MSS", "down")];
  const mk = (i) => [ev(i, "BOS", "up"), ev(i + 2, "MSS", "down")];
  const a = scoreEvents(mk(10), eng(10), candles, { windowCandles: 2 });
  const b = scoreEvents(mk(30), eng(30), candles, { windowCandles: 2 });

  assert.equal(a.recall, b.recall, "نفس الدليل لازم يعطي نفس الرقم مهما كان موقعه الزمني");
  assert.equal(a.truePositives, b.truePositives);
});

test("f1 ما بتقدر تناقض precision و recall", () => {
  /* العيب: f1 كانت تنحسب من tp/fp/fn، وخطأ الاتجاه ما بيدخل بأي منهم،
     فكانت تطلع precision 0.5 · recall 0.5 · f1 1.0 بنفس السطر. */
  const refs = [ev(10, "BOS", "up"), ev(30, "BOS", "up")];
  const eng = [ev(10, "BOS", "up"), ev(30, "BOS", "down")];
  const s = scoreEvents(refs, eng, candles, { windowCandles: 2 });

  assert.equal(s.directionErrors, 1);
  assert.equal(s.precision, 0.5);
  assert.equal(s.recall, 0.5);
  assert.equal(s.f1, 0.5, `f1 ${s.f1} بتناقض precision ${s.precision} و recall ${s.recall}`);

  // الادعاء العام: f1 دايماً المتوسط التوافقي للمنشورَين
  const expected = +((2 * s.precision * s.recall) / (s.precision + s.recall)).toFixed(4);
  assert.equal(s.f1, expected);
});

test("مطابقة نوعها واتجاهها غلط الاتنين = FN + FP، مش مطابقة", () => {
  const refs = [ev(20, "BOS", "up")];
  const eng = [ev(20, "MSS", "down")];
  const s = scoreEvents(refs, eng, candles, { windowCandles: 2 });

  assert.equal(s.details.matches.length, 0, "ما لازم ينبني زوج");
  assert.equal(s.falseNegatives, 1);
  assert.equal(s.falsePositives, 1);
  assert.equal(s.f1, 0);
});
