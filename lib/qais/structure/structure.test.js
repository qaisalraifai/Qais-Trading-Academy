...
/* ============================================================================
   ...
   ============================================================================ */
test("المستوى ما بينكسر قبل لحظة تأكيده الفعلية", () => {
  /* ⚠️ النسخة الأولى كانت بتفحص الحد الضعيف `swingRef.index + lookback`.
     بس المحرك بيبوّب على `confirmedAtIndex` (زجزاج التأكيد)، واللي بيجي
     **أبعد بكتير**: مثال حقيقي — حدث@31 · swingRef@11 · الحد الضعيف 13
     · الحد الحقيقي 23. يعني حذف البوابة الحقيقية كان بيخلّي الاختبار
     أخضر، وبيرجّع الاكتشاف الرجعي اللي سبّب «MSS بتأخير ١٩ شمعة»
     و٦ أحداث تختفي. */
  clock = 1700000000;
  const c = path([100, 150, 122, 178, 130, 165], 12);
  const r = analyzeStructureV2(c, { timeframe: "h4", lookback: 2 });

  assert.ok(r.events.length > 0, "لازم يطلع أحداث");
  const byIndex = new Map(r.majorSwings.map((s) => [s.index, s]));

  let checkedStrict = 0;
  for (const e of r.events) {
    if (!e.swingRef) continue;
    const swing = byIndex.get(e.swingRef.index);
    assert.ok(swing, `حدث@${e.index} بيشير لسوينغ مش بالقائمة المؤكَّدة`);
    assert.ok(
      Number.isFinite(swing.confirmedAtIndex),
      `السوينغ @${swing.index} بلا لحظة تأكيد — البوابة الحقيقية مفقودة`
    );
    assert.ok(
      e.index >= swing.confirmedAtIndex,
      `حدث@${e.index} كسر مستوى ما تأكد إلا عند ${swing.confirmedAtIndex}`
    );
    // والحد الحقيقي لازم يكون أشد من الضعيف، وإلا الاختبار ما بيحرس شي
    if (swing.confirmedAtIndex > swing.index + r.meta.config.lookback) checkedStrict++;
  }
  assert.ok(checkedStrict > 0, "كل الأحداث صادف إنها على الحد الضعيف — الاختبار ما فحص البوابة الحقيقية");
});
...
test("الاتجاه بينقلب لحظة الـMSS — ما بينعلّق", () => {
  /* ⚠️ النسخة الأولى من هالاختبار كانت **بتنفّذ صفر assertion**: حارسين
     متداخلين (`if (mss.length)` ثم `if (nextSameDir)`)، والـMSS طلع آخر
     حدث فما كان في خلفه شي — فالفحص ما بيوصله والاختبار بينجح فاضي.
     وهاد الاختبار الوحيد اللي بيغطي القاعدة المركزية.

     التصحيح: مسار فيه MSS **وبعده** كسر بنفس الاتجاه، مع تأكيد الشروط
     المسبقة نفسها قبل الفحص. */
  clock = 1700000000;
  const c = path([100, 150, 128, 175, 120, 145, 95], 12);
  const r = analyzeStructureV2(c, { timeframe: "h4" });

  const mss = r.events.filter((e) => e.type === "MSS");
  assert.ok(mss.length >= 1, `المسار لازم يولّد MSS — طلع ${r.events.map((e) => e.type + ":" + e.direction).join(",")}`);

  const first = mss[0];
  const after = r.events.filter((e) => e.index > first.index && e.direction === first.direction);
  assert.ok(after.length >= 1, "لازم يكون في كسر تالي بنفس اتجاه الـMSS حتى ينفحص الانقلاب");

  /* بعد MSS هابط، أي كسر هابط تالي لازم يكون BOS (استمرار بالاتجاه
     الجديد) — دليل إنه الاتجاه انقلب فعلاً لحظة الـMSS مش بعدين. */
  assert.equal(after[0].type, "BOS", "الاتجاه ما انقلب عند الـMSS");

  // والحالة المنشورة لازم توافق: MSS = مرحلة تحوّل واتجاهها هو الجديد
  const lastEv = r.events[r.events.length - 1];
  assert.equal(r.state.trend, lastEv.direction);
});
...
test("كل حدث بيحمل الحقول المطلوبة كاملة", () => {
  ...
  for (const e of r.events) {
    ...
    assert.ok(["BOS", "MSS"].includes(e.type), `نوع غير معروف: ${e.type}`);
...
/* ============================================================================
   ٨.ب) حراس انحدار لعيوب أثبتتها مراجعة مستقلة
   ============================================================================ */
test("ATR ما بيسرّب مستقبل: تعديل شمعة لاحقة ما بيغيّر قيمة سابقة", () => {
  /* العيب: atrAt كانت تمسح **للأمام** لما القيمة ناقصة، فأي فهرس قبل
     اكتمال الفترة كان بياخد قيمة محسوبة من شموع ما وصلناها. */
  const mk = (bump) =>
    Array.from({ length: 40 }, (_, i) => {
      const big = i === 12 && bump;
      return { time: i, open: 100, high: big ? 140 : 101, low: big ? 60 : 99, close: 100 };
    });

  const a = atrSeries(mk(false), 14);
  const b = atrSeries(mk(true), 14);

  for (let i = 0; i < 12; i++) {
    assert.equal(atrAt(a, i), atrAt(b, i), `الفهرس ${i} تأثّر بشمعة مستقبلية`);
  }
  assert.equal(atrAt(a, 3), null, "قبل اكتمال الفترة ما في قيمة — null مش تخمين");
});

test("السوينغ ما بيتأكد بدون ATR بدل ما تصير العتبة صفر", () => {
  /* العيب: `(atrAt(...) ?? 0) * atrMult` بتخلي العتبة صفر بمنطقة الإحماء،
     فكل بيفوت بيصير سوينغ خارجي بلا فحص دلالة.

     الادعاء الصح: التأكيد بيصير عند **البيفوت المعاكس** (confirmedAtIndex
     ناقص lookback)، فلازم ATR يكون متوفر هناك — مش عند السوينغ نفسه،
     لأنه سوينغ قديم بينفع يتأكد ببيفوت لاحق. */
  clock = 1700000000;
  const c = path([100, 145, 118, 162, 130, 175], 12);
  const period = 14;
  const r = analyzeStructureV2(c, { timeframe: "h4", atrPeriod: period });
  const atr = atrSeries(c, period);
  const lookback = r.meta.config.lookback;

  assert.ok(r.majorSwings.length > 0, "لازم يتأكد سوينغات");
  for (const s of r.majorSwings) {
    const confirmingPivot = s.confirmedAtIndex - lookback;
    assert.ok(
      Number.isFinite(atr[confirmingPivot]) && atr[confirmingPivot] > 0,
      `سوينغ @${s.index} تأكد ببيفوت @${confirmingPivot} وATR هناك ${atr[confirmingPivot]}`
    );
  }
});

test("state.trend = اتجاه آخر حدث، مش قراءة التسميات", () => {
  /* العيب: كان في مفهومان للاتجاه — واحد بينقلب عند الـMSS (صح) وواحد
     منشور من التسميات، فكان state.trend يقول up بعد MSS هابط. */
  clock = 1700000000;
  const c = path([100, 150, 128, 172, 140, 190, 120], 12);
  const r = analyzeStructureV2(c, { timeframe: "h4" });

  const last = r.events[r.events.length - 1];
  assert.ok(last, "لازم يطلع أحداث");
  assert.equal(r.state.trend, last.direction, `آخر حدث ${last.type}:${last.direction} بس trend ${r.state.trend}`);

  const mss = r.events.filter((e) => e.type === "MSS");
  if (mss.length) {
    const m = mss[mss.length - 1];
    const after = r.events.filter((e) => e.index > m.index);
    if (after.length === 0) assert.equal(r.state.trend, m.direction, "الاتجاه ما انقلب مع آخر MSS");
  }
});

test("الأحداث ما بتنبلع لما التسلسل ما يكتمل", () => {
  /* العيب: خروج مبكّر عند majorSwings < 4 كان يرجّع INSUFFICIENT_DATA
     بدون lastEvent — فحدث مؤكَّد بيختفي من الحالة كلياً. */
  clock = 1700000000;
  const c = [...ramp(100, 140, 16), ...ramp(140, 124, 10), ...ramp(124, 154, 14), ...ramp(154, 118, 16)];
  const r = analyzeStructureV2(c, { timeframe: "h4" });

  if (r.events.length > 0) {
    assert.ok(r.state.lastEvent, "الحالة لازم تحمل آخر حدث مهما كان عدد السوينغات");
    assert.equal(r.state.lastEvent.type, r.events[r.events.length - 1].type);
    assert.equal(r.state.trend, r.events[r.events.length - 1].direction);
    assert.ok(r.state.lastPrice != null, "آخر سعر لازم يكون موجود");
    assert.equal(typeof r.state.sequenceEstablished, "boolean");
  }
});
...
/* ============================================================================
   ٩.ب) حدود المصفوفة: أول شمعة وآخر شمعة
   ============================================================================ */
test("حدث على آخر شمعة بينطلع، وبدون عوامل بتحتاج شمعة بعدها", () => {
  clock = 1700000000;
  /* المسار بينتهي بكسر — يعني شمعة الكسر هي آخر شمعة بالمصفوفة. */
  const c = [...ramp(100, 145, 16), ...ramp(145, 122, 12), ...ramp(122, 158, 16)];
  const r = analyzeStructureV2(c, { timeframe: "h4" });
  const lastIdx = c.length - 1;

  for (const e of r.events) {
    assert.ok(e.index <= lastIdx, `حدث عند ${e.index} برّا المصفوفة`);
    if (e.index === lastIdx) {
      /* الكسر الكاذب بيحتاج شموع بعدها — على آخر شمعة ما بينحكم عليه. */
      assert.equal(e.fakeBreak, false, "ما بينفع يُحكم بكسر كاذب بدون شموع بعده");
    }
  }
  assert.equal(r.state.lastPrice, c[lastIdx].close);
});

test("الزخم بيصرّح بالعوامل غير المتاحة على الحدود بدل ما يحسبها فشل", () => {
  clock = 1700000000;
  const c = ramp(100, 130, 40);

  const first = classifyDisplacement(c, 0);
  assert.ok(first.unavailable.includes("gap"), "الفجوة بتحتاج شمعة قبل وبعد");

  const last = classifyDisplacement(c, c.length - 1);
  assert.ok(last.unavailable.includes("gap"), "آخر شمعة ما إلها شمعة بعدها");
  const gapFactor = last.factors.find((f) => f.name === "gap");
  assert.equal(gapFactor.passed, null, "غير متاح = null مش false");
});

test("تحليل مصفوفة متزايدة: كل نتيجة صالحة بحالها بدون انهيار", () => {
  clock = 1700000000;
  const c = [...ramp(100, 145, 16), ...ramp(145, 120, 12), ...ramp(120, 160, 16)];
  for (let n = 30; n <= c.length; n += 7) {
    const r = analyzeStructureV2(c.slice(0, n), { timeframe: "h4" });
    assert.equal(r.ok, true, `فشل عند ${n} شمعة`);
    assert.ok(r.state.state, "لازم حالة معرّفة");
    for (const e of r.events) assert.ok(e.index < n, `حدث عند ${e.index} وعدد الشموع ${n}`);
  }
});

/* ============================================================================
   ٩.ج) تبديل الفريم — نفس الشكل بفريمات مختلفة
   ============================================================================ */
test("نفس الشكل على فريمات مختلفة بيعطي نفس الهيكل", () => {
  /* الفريم بيغيّر أوقات الشموع بس، مش شكل الحركة. المحرك ما بيقرا الوقت
     إطلاقاً بقرار الهيكل — فالنتيجة لازم تكون واحدة. لو اختلفت، معناها
     في اعتماد خفي على المدة الزمنية. */
  const shape = [100, 150, 122, 178, 130, 165];
  const build = (secs) => {
    clock = 1700000000;
    const c = path(shape, 12);
    return c.map((x, i) => ({ ...x, time: 1700000000 + i * secs }));
  };

  const results = [300, 900, 3600, 14400, 86400].map((secs) =>
    analyzeStructureV2(build(secs), { timeframe: String(secs) })
  );

  const sig = (r) => r.events.map((e) => `${e.type}:${e.direction}@${e.index}`).join("|");
  for (let i = 1; i < results.length; i++) {
    assert.equal(sig(results[i]), sig(results[0]), "الهيكل اختلف بتبديل الفريم");
    assert.equal(results[i].majorSwings.length, results[0].majorSwings.length);
    assert.equal(results[i].state.state, results[0].state.state);
  }
  assert.ok(results[0].events.length > 0, "لازم يطلع أحداث حتى يكون الفحص معنيّ");
});
