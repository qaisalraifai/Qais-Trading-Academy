...
/* ============================================================================
   ٦) BOS مقابل CHOCH مقابل MSS — تعريفات منفصلة فعلاً
   ============================================================================ */
test("كسر بعكس الاتجاه القائم بينصنّف CHOCH مش BOS", () => {
  clock = 1700000000;
  const c = [
    ...ramp(100, 140, 16), // صعود
    ...ramp(140, 122, 10), // قاع محمي ~122
    ...ramp(122, 152, 14), // BOS صاعد
    ...ramp(152, 118, 18), // كسر القاع بعكس الاتجاه = CHOCH
    ...ramp(118, 126, 8),
  ];
  const r = analyzeStructureV2(c, { timeframe: "h4" });
  const types = r.events.map((e) => `${e.type}:${e.direction}`);
  assert.ok(types.includes("CHOCH:down"), `ما طلع CHOCH هابط — الأحداث: ${types.join(", ")}`);
});

test("MSS ما بيطلع إلا بعد BOS مؤكِّد، وبيشير للـCHOCH وللمؤكِّد", () => {
  clock = 1700000000;
  const c = [
    ...ramp(100, 140, 16),
    ...ramp(140, 122, 10),
    ...ramp(122, 152, 14),
    ...ramp(152, 116, 18), // CHOCH هابط
    ...ramp(116, 130, 8), // ارتداد
    ...ramp(130, 104, 14), // BOS هابط مؤكِّد
    ...ramp(104, 110, 6),
  ];
  const r = analyzeStructureV2(c, { timeframe: "h4" });
  ...
  for (const m of mss) {
    assert.ok(m.confirmedBy, "MSS لازم يحمل مرجع التأكيد");
    assert.ok(m.confirmedBy.index > m.index, "التأكيد لازم يكون بعد لحظة الـCHOCH");
    assert.ok(m.chochId, "MSS لازم يشير للـCHOCH اللي انبنى عليه");
    assert.equal(m.fakeBreak, false, "كسر كاذب ما بيترقّى لـMSS");
  }
});
