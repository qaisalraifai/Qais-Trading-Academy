...
/**
 * قيمة ATR عند فهرس معيّن — **بدون أي نظر للمستقبل**.
 *
 * ⚠️ النسخة السابقة كانت تمسح **للأمام** لما القيمة ناقصة، بحجة إنه بداية
 * البيانات تضل قابلة للتحليل. النتيجة كانت تسريب مستقبل مباشر: أي فهرس
 * قبل اكتمال الفترة كان بياخد قيمة محسوبة من شموع المستدعي ما وصلها بعد.
 * مُثبت بالتجربة — تكبير الشمعة ١٢ وحدها بيغيّر ATR المُبلَّغ عند الفهرس ٣:
 *
 *     بدون تعديل       : 2.0000
 *     بعد تكبير شمعة ١٢: 7.5714
 *
 * وهاي القيمة بتغذّي عتبة تأكيد السوينغ الخارجي وحساب حسم الكسر والثقة —
 * يعني قرار «هاد سوينغ معتبر» كان ممكن ينبني على بيانات مستقبلية.
 *
 * المسح للخلف بيضل مسموح: قيمة أقدم معلومة وقتها فعلياً، فهي سببية.
 * قبل اكتمال الفترة ما في ولا قيمة سابقة، فبيرجع `null` — والمستدعي لازم
 * يصرّح بعدم القدرة على القياس بدل ما يخمّن.
 */
