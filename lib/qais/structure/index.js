...
export const DEFAULTS = {
  ...
  atrPeriod: 14,
  majorAtrMult: 1.5, // عتبة دلالة الساق الخارجية
  majorMinLegBars: 3,
  fakeBreakBars: 3,
...
export function analyzeStructureV2(candles, options = {}) {
  ...
  const { events, wickBreaks } = detectEvents(candles, majorSwings, {
    ...
    atrPeriod: cfg.atrPeriod,
    mssMinDisplacement: cfg.mssMinDisplacement,
  });
  ...
  return {
    ...
    meta: {
      ...
      counts: {
        ...
        choch: events.filter((e) => e.type === "CHOCH").length,
...
function deriveState(majorSwings, events, candles) {
  ...
  if (trend === "up" || trend === "down") {
    /* بعد BOS بنفس الاتجاه = توسّع. بعد CHOCH معلّق = تحوّل.
       غير هيك (السعر راجع داخل الساق) = تصحيح. */
    if (last?.type === "CHOCH") state = "TRANSITION";
    else if (last?.type === "BOS" && last.direction === trend) state = "EXPANSION";
    else if (last?.type === "MSS") state = "EXPANSION";
