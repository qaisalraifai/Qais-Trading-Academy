...
export function analyzeOrderBlocksV2(candles, options = {}) {
  ...
  for (let i = 1; i < candles.length; i++) {
    ...
    /* ⚠️ من `i` مش `i + 1` — شمعة الزخم نفسها بتدخل بفحص الإبطال.
       شمعة صاعدة فتحت بفجوة هابطة بتقدر تسكّر تحت كتلة الطلب، وكانت
       تنعدّ كتلة سليمة لأن الفحص كان بيبلّش بعدها. (نفس التصليح بـrules-sk.js) */
    const invalidIndex = firstInvalidationIndex(candles, i, built.levels.invalidation, dirUp);
