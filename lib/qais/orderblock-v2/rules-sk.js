...
export function analyzeOrderBlocksSK(candles, options = {}) {
  ...
  const rejected = { noGroup: 0, wickBlock: 0, firstEventOpposite: 0, noEvent: 0, noFvg: 0, noLiquidity: 0, duplicate: 0 };
  ...
  for (let i = 1; i < candles.length; i++) {
    ...
    const confirmedAtIndex = Math.max(event.index, fvg ? fvg.confirmedAtIndex : 0, i);
