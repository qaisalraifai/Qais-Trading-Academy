...
export const SETUP_DEFAULTS = {
  /* ── نوافذ البحث ──────────────────────────────────────────────────
     نصّه «بعدها بيصير SMT… أول ما يصير CISD وندخل» ما فيه **مدة**.
     فالمدة كانت لازم تنسأل، وانسألت.

     ✅ **نافذة الـSMT قراره** (٢٠٢٦-٠٨-٢٢). انعرضت عليه الحالة الملموسة:
     «السعر لمس الكتلة ١٢ الظهر، وصار SMT ١١ بالليل — بعد ١١ ساعة. هاي
     بتقبلها؟» وجوابه: **«بقبلها»**. فـ٤٨ شمعة M15 (١٢ ساعة) صارت مرجعاً
     مش اختياراً.

     ⚠️ اللي **ما** انحسم: الحدّ **الأعلى**. جوابه بيثبّت إنه ١١ ساعة
     مقبولة — يعني النافذة مش أقصر من هيك. ما بيقول وين بتنتهي. على
     العيّنة القائمة الفرق ما بيعضّ (٩٦ شمعة = ٢٤ ساعة بتعطي **نفس** الست
     صفقات)، فالسؤال نظري لهلق.

     ⚠️ التلاتة **ما بينعايروا** — تعيير = اختيار الرقم اللي بيطلّع نتيجة
     أحلى، وهاد ممنوع بقواعد المشروع. الأثر مقيس على ١٠٥ كتلة:

       smtSearchBarsLower   ٩٦→٦ · ٤٨→٦ · ٢٤→٣ · ١٢→٣ · ٦→٠ صفقة
       cisdMaxBars          ٢٤٠→٦ · ٦٠→٦ · ١٥→٥ · ٦→٥ صفقة
       smtSearchBars        ١→٦ · ١٢→٦ · ٥٠٠→٦ صفقة   ← بلا أثر

     يعني `smtSearchBarsLower` **بيحدّد عدد الصفقات**، و`smtSearchBars`
     ميت بالإعداد القائم (بينستعمل بس لما الـSMT على فريم الكتلة نفسه،
     وهو دايماً على M15 هون). و`cisdMaxBars` لسا **بلا مرجع** — ما
     انسأل عنه، بس أثره أضعف.

     ⚠️ **وقرار النافذة ما بيحلّ ملاحظة تانية**: بوابة الـSMT ما رفضت ولا
     كتلة من الـ٧ اللي وصلتها، وبفحص عدمي مترابط مزحزح ٣٠ يوم لسا بيعطي
     SMT بـ٤/٧. النافذة صارت مشروعة، بس السؤال «هل الكشف بيميّز فعلاً؟»
     مفتوح — وهو عن **تعريف الـSMT** مش عن مدتها. */

  /* أقصى شموع (بفريم الكتلة) بينتظرها بين اللمس والـSMT —
     بينطبّق بس لما `smtPrimary.candles === candles`. */
  smtSearchBars: 12,
  /* أقصى شموع بينتظرها بين اللمس والـSMT لما الـSMT على فريم أصغر.
     ٤٨ شمعة M15 = **١٢ ساعة** — بقراره (فوق).
     ⚠️ كان **سطرياً** جوّا التعبير بلا اسم بالإعدادات — انرفع لهون بلا
     تغيير قيمة، عشان الرقم اللي بيحكم النتيجة يكون مرئياً. */
  smtSearchBarsLower: 48,
  /* أقصى شموع (بالفريم الأصغر) بينتظرها بين الـSMT والـCISD.
     ⚠️ بلا مرجع — ما انسأل عنه. */
  cisdMaxBars: 60,
  ...
  /* ── هامش الستوب تحت نقطة الـSMT ──────────────────────────────────
     ...
     ✅ **مُشغَّل على `0.25`** — قراره (٢٠٢٦-٠٨-٢١): «شغل الهامش 0.25».

     الرقم **قراره مش استنتاجي**. كان صفراً من ٢٠٢٦-٠٨-٢٠ لما رفض يثبّت
     مضاعفاً على N=١ («لا تعتمد 1× أو 0.5× أو 0.25× كقاعدة ثابتة من حالة
     واحدة»)، وثبّته اليوم بعد ما صارت نقطة الـSMT **القاع الصامد** وظهرت
     صفقتان بمخاطرة ٧ و٢٦ نقطة (0.11× · 0.48× ATR).

     ⚠️ الهامش بينحسب على **نقطة الـSMT** لأنها المصدر القائم؛ لو انبدّل
     المصدر لحد الإبطال بينطبّق عليه. */
  ...
  /* ── مصدر الستوب ──────────────────────────────────────────────────
     ...
     ⚠️ **انشغّل ساعة واحدة وانرجع** (٢٠٢٦-٠٨-٢١). انشغّل بقراره «شغل حد
     الابطال للستوب»، وبعدها بعت صورة وشرح كيف الستوب بيتحدّد فعلياً:
     «بنخلي الستوب تحت القاع يلي ما نزل النازداك تحته» — يعني تحت نقطة
     الـSMT بتعريفها الجديد (القاع الصامد)، مش تحت حد إبطال الكتلة.
     الفرق مقيس على صورته: مخاطرته ٣٠٨ نقطة، وحد الإبطال بيعطي ٧٧٠–١١٠٥.

     فالمصدر رجع `"smt"`، و`"block"` انترك متاحاً. */
  stopSource: "smt",
};
...
export function buildTradeSetup(block, ctx, options = {}) {
  ...
  const sTo = smtPrimary.candles === candles
    ...
    : Math.min(smtPrimary.candles.length - 1, sFrom + cfg.smtSearchBarsLower);
  ...
  /* ── الترتيب: **SMT أولاً، وبعدها CISD** ──────────────────────────
     نصّه (٢٠٢٦-٠٨-٢١): «بيصير SMT… **هون بنستنى** CISD على ناسداك
     وبندخل». يعني الـCISD لازم تكون **بعد** الـSMT اللي بنتداول عليها.

     ⚠️ الترتيب القديم كان معكوس: بيبلّش بحث الـCISD من **أول** SMT،
     وبعدين بيرجع بيختار أعمق SMT قبل الـCISD. ذاك اشتغل لما كانت نقطة
     الـSMT **امتداد كنس** (تحت السعر دايماً). بس بعد ما صارت النقطة
     **القاع الصامد** (فوق السعر عادةً)، الترتيب المعكوس بيطلّع ستوباً
     **فوق** الدخول — مقيس: ستوب 27,787.15 مقابل دخول 27,461.35.

     فبنجرّب الـSMT بالترتيب الزمني، وبندوّر CISD بعد كل وحدة، وبناخد
     **أول تركيبة صالحة** (الستوب بالجهة الصح). */
  const cisdOpts = {
  ...
  let smt = null, cisd = null, lastReject = null;
  for (const cand of smts) {
    const tSmt = smtPrimary.candles[cand.atIndex].time;
    const from = cisdFrame.candles.findIndex((c) => c.time >= tSmt);
    if (from < 0) continue;

    const c = findCISD(cisdFrame.candles, from, dirUp, cisdOpts);
    if (!c || c.value === "INSUFFICIENT_DATA") continue;

    /* الستوب من هالـSMT — لازم يطلع بالجهة الصح من دخولها هي. */
    const base = cfg.stopSource === "block"
      ? (block.levels.invalidation ?? block.levels.outerWick)
      : cand.point;
    if (dirUp ? base < c.breakPrice : base > c.breakPrice) { smt = cand; cisd = c; break; }
    lastReject = { stop: base, entry: c.breakPrice };
  }

  if (!cisd) {
    if (lastReject) {
      return blocked("stop",
        `الستوب ${lastReject.stop.toFixed(2)} مش ${dirUp ? "تحت" : "فوق"} الدخول ${lastReject.entry.toFixed(2)} — ` +
        `ولا SMT بالنافذة أعطت ترتيباً صالحاً`,
        { touch, smtCount: smts.length });
    }
    return blocked("cisd", `ما صار CISD خلال ${cfg.cisdMaxBars} شمعة بعد أي SMT`, { touch, smtCount: smts.length });
  }
  ...
  return {
    ...
    entry,
    stop,
    /* ⚠️ المصدر بينطلع بالمخرج — الشارت كان بيكتب «SL · SMT» ثابتاً،
       فلما تبدّل المصدر صارت التسمية بتكذب على الطالب. */
    stopSource: cfg.stopSource,
    stopLabel: cfg.stopSource === "block" ? "حد إبطال الكتلة" : "نقطة الـSMT",
    risk: +risk.toFixed(5),
