...
import { detectSMT } from "./smt-v2.js";
...
export function buildTradeSetup(block, ctx, options = {}) {
  ...
  /* ⚠️ مصدر الـSMT **بينمرّر** — ما بينفرض.
     ...
     المقياس بيغيّر الجواب، فاختياره قرار منهجي مش تفصيل تنفيذي. الوحدة
     بتاخده كمدخل وبتسجّله بالمخرج. */
  const smtPrimary = ctx.smtPrimary ?? { candles, swings: structure.majorSwings, scale: "major", timeframe: "block" };
  const smtCorrelate = ctx.smtCorrelate ?? correlate;
  ...
  /* ── ٤ · CISD على الفريم الأصغر ──────────────────────────────────── */
  if (!lower?.candles?.length) {
    return insufficient("ما انمرّر فريم أصغر — CISD غير قابلة للتقييم", "cisd");
  }
  let from = lower.candles.findIndex((c) => c.time >= firstSmtTime);
  if (from < 0) {
    return insufficient("لحظة الـSMT برّا نطاق الفريم الأصغر", "cisd");
  }
  const cisd = findCISD(lower.candles, from, dirUp, { maxBarsToBreak: cfg.cisdMaxBars });
  ...
  return {
    ...
    chain: {
      ...
      cisd: { index: cisd.index, time: cisd.time, level: cisd.level, breakPrice: cisd.breakPrice,
              timeframe: lower.timeframe ?? null, reason: cisd.reason },
    },
    entry,
    stop,
    risk: +risk.toFixed(5),
