  ...
  try {
    ...
    if (lastTrade?.ok) {
      ...
      chartTrade = {
        ...
        stopLabel: t.stopLabel ?? null,
        risk: t.risk,
        targets: t.targets ?? null,
        targetsStage: t.targetsStage ?? null,
        displayTF: cfg.blockTF,
        /* ⚠️ الشارت بيكتب «SMT: غير متحقق» لما الحقل مفقود. السلسلة **ما
           بتكمل** بلا SMT أصلاً، فوجود الصفقة نفسه هو الدليل — بس الحقل
           كان ناقصاً فطلعت التسمية غلط على صفقة SMT-ها متحقق. */
        smtVerified: !!t.chain?.smt,
        smtPoint: t.chain?.smt?.point ?? null,
        /* حدود الكتلة للصندوق التوضيحي — كان بيقرا `obZone` وما بينوجد. */
        obZone: t.block?.levels
          ? {
              top: Math.max(t.block.levels.open, t.block.levels.close, t.block.levels.outerWick),
              bottom: Math.min(t.block.levels.open, t.block.levels.close, t.block.levels.outerWick),
            }
          : null,
        outcome: {
