...
export const ADAPTER_DEFAULTS = {
  ...
  blockTF: "h4",
  /* فريم الدخول: SMT وCISD. */
  entryTF: "m15",
  /* أقصى كتل حيّة بتنفحص سلسلتها — من الأحدث. */
  maxBlocks: 5,
  minCandles: 60,
};
...
export function runSkV2({ symbol, candlesByTF, correlatedByTF = null }, options = {}) {
  ...
  try {
    ...
    for (const b of live) {
      ...
      setups.push({ blockId: b.id, direction: b.direction, levels: b.levels, setup: s });
    }
    ...
    return {
      ...
      notes: [
        ...
        ctx.lower ? null : `ما في شموع ${cfg.entryTF} — CISD غير قابلة للتقييم`,
      ].filter(Boolean),
