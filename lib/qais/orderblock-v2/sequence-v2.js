 ...
 * B غلط وساقاً أقصر.
 */
/* ⚠️ من وثيقته «SK System – Part 1»، القانون ٣:
   «الموجة B يجب على الأقل أن تصحّح للنسبة 0.382 من أداة تصحيح الفيبوناتشي».

   رقم **منصوص عليه بوثيقته**، مش معايَر ولا مستنتَج. قبله كانت الوحدة
   بتاخد أعمق تصحيح أياً كان عمقه — يعني ارتداد ٥٪ كان بيمرق كـB. */
export const MIN_B_RETRACEMENT = 0.382;

export function enumerateSequences(candles, majorSwings, asOfIndex, options = {}) {
  const minRetrace = options.minRetracement ?? MIN_B_RETRACEMENT;
  const sw = confirmedUpTo(majorSwings, asOfIndex);
  const out = [];
  ...
  for (let i = 0; i + 1 < sw.length; i++) {
    for (const dirUp of [true, false]) {
      ...
      /* ⚠️ قانونه ٣: الموجة B لازم تصحّح **٠.٣٨٢ على الأقل** من الساق. */
      const retrace = Math.abs(A.price - B.price) / leg;
      ...
      out.push({
        ...
        /* نسبة تصحيح B — بتنسجّل عشان القاعدة تضل قابلة للمراجعة. */
        retracement: +retrace.toFixed(4),
        breakIndex: brk,
