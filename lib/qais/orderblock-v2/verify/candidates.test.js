...
test("القواعد المحسومة مقيَّمة لكل حالة", () => {
  ...
  for (const c of result.cases) {
    ...
    for (const r of ["R3", "R4", "R5", "R6"]) {
      assert.equal(typeof sr[r], "boolean", `${c.id}.${r} مش بوليان`);
    }
    /* R6 لازم يطابق قياس الاكتمال — مش حكم مستقل. */
    assert.equal(sr.R6, c.measurements.completedAfterBars !== null, `${c.id}: R6 ما بيطابق القياس`);
...
test("ولا حالة معروضة بالأداة بتخالف قاعدة محسومة", () => {
  ...
  /* ⚠️ الجولة التانية بتعرض حالات **جديدة** — اللي انسمّت بتنستثنى
     عشان التسميات تضيف بيانات بدل ما تعيد نفسها. فالفحص صار: ولا حالة
     معروضة مسمّاة سلفاً، والحالات الحاسمة مفروضة. */
  const labeled = new Set((REF.labels || []).map((l) => l.caseId));
  const decisive = new Set(shown.decisiveIncluded || []);
  for (const s of shown.cases) {
    if (decisive.has(s.id)) continue;
    assert.ok(!labeled.has(s.id), `${s.id} انسمّت سلفاً وراجعة بالعيّنة`);
  }
  for (const id of decisive) {
    assert.ok(shown.cases.some((s) => s.id === id), `الحالة الحاسمة ${id} مش بالعيّنة`);
  }
});
