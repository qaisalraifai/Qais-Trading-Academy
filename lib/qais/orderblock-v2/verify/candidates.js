...
/** حد R4 — «جسم أكبر من ذيول» = المعنى الحرفي لـ«ما تكون عبارة عن ذيل». */
export const MIN_BLOCK_BODY_RATIO = 0.5;

/* ⚠️ هاد **قرار عيّنة**، مش قاعدة منهجية. الفرق مهم.
   ---------------------------------------------------------------------------
   R5 بالمحرك قاعدة **دورة حياة**: الكتلة قائمة لحد ما يسكّر السعر خلفها.
   ما إلها نافذة، وهيك نطقها صاحب المنهجية بالحرف. المحرك بينفّذها هيك.

   بس **لأداة التسمية** فيه شرط عملي: كتلة ماتت بعد ٣ شموع والسعر كمّل
   بالعكس ما بتعطي إشي يُحكم عليه. صاحب المنهجية وقف عند وحدة منهن (C256:
   شمعة خضرا ٣٢٩ نقطة، «حركة» حمرا ١٠٥ نقطة، وانكسرت بعد ٣ شموع والسعر
   كمّل صعود) وقال هاي منطقة غلط — وهو محق.

   ليش ٥ تحديداً: هي **أصغر** نافذة بتشيل هالحالة. والأهم إنها بتحافظ على
   الصنفين بالعيّنة (٦٤ «أول حدث بنفس الاتجاه» مقابل ٥٤ معاكس) — بينما
   الربط بأول حدث هيكل بيعطي ٤١ مقابل ١، يعني بيحسم الزخم سلفاً وما بيضل
   إشي يتسمّى.

   ⚠️ الرقم مكشوف ومعزول هون بقصد. تغييره بيغيّر **مين بينعرض للتسمية**
   وبس — ما بيمس المحرك ولا أي قاعدة محسومة. */
export const LABELING_MIN_SURVIVAL_BARS = 5;
...
export function collectDisplacementCases(candles, options = {}) {
  ...
  for (let i = 1; i < candles.length - 1; i++) {
    ...
    const { levels } = built;
    /* من `i` — شمعة الحركة نفسها داخلة بالفحص (نفس تصليح R5 بالمحركين). */
    const invalidationIndex = firstInvalidationIndex(candles, i, levels.invalidation, dirUp);
    ...
    /* R6 — **اكتمال** الكتلة. قرار صاحب المنهجية (٢٠٢٦-٠٨-١٨):
       «أول شمعة من سلسلة الشموع الهابطة، يعني أعلاهم سعراً، بس يغلق السعر
        فوقها — هذا آخر شرط من شروط تكوّن الـOB.»

       أول شمعة بالمجموعة جسمها الأبعد سعرياً، وفتحها هو **مستوى Open**.
       فالشرط: إغلاق خلف `levels.open` قبل الإبطال. بالجسم مش بالذيل —
       متّسق مع «الكسر بالإغلاق» بكل المنهجية.

       ⚠️ هاد شرط تكوّن مش دورة حياة: قبل ما يتحقق، الكتلة **ما تكوّنت
       أصلاً** — مش «تكوّنت وضعيفة». */
    ...
    cases.push({
      ...
      measurements: {
        ...
        extLegBars: legBars,
        /* كم شمعة صمدت الكتلة من لحظة الحركة لأول إغلاق خلفها.
           `null` = ما انكسرت لآخر البيانات. */
        survivedBars: invalidationIndex === -1 ? null : invalidationIndex - i,
        /* كم شمعة لحد ما اكتملت الكتلة (R6). `null` = ما اكتملت أبداً. */
        completedAfterBars: completedAtIndex === null ? null : completedAtIndex - i,
      ...
      /* ── القواعد **المحسومة** — بتنقيّم لكل حالة ─────────────────────
         ⚠️ التقييم بينتسجّل، والحالة **ما بتنرمى** من هون.
         `collectDisplacementCases` بتضل ترجّع كل الحالات عشان معرّفاتها
         تضل ثابتة — المرجع البشري مربوط فيها. التصفية بتصير عند أخذ
         العيّنة (`onlySettled`)، فتغيير قاعدة ما بيزحزح ولا معرّف.

         R1 (الزخم) **مش هون** — هو بالضبط اللي عم نتعلّمه من التسميات،
         وتصفيته بتقرر النتيجة قبل ما يشوفها. R2 (السيولة) معطّلة لأن
         تعريفها الحالي بيتحقق ١٠٠٪. */
      settledRules: {
        /* R3 — كل شموع المجموعة معاكسة لاتجاه الحركة. */
        R3: group.candles.every((gc) => (gc.close < gc.open) === dirUp),
        /* R4 — الكتلة ككل جسم مش ذيل. */
        R4: groupRange > 0 && Math.abs(gm.open - gm.close) / groupRange > MIN_BLOCK_BODY_RATIO,
        /* R5 — شمعة الحركة نفسها ما بتسكّر خلف الذيل الطرفي.
           بلا عدد شموع صمود: أي رقم هون بيكون مخترع. الفحص على الشمعة
           اللي «بتكوّن» الكتلة وبس — وهي نفس الثغرة اللي انصلحت بالمحرك. */
        R5: !(dirUp ? c1.close < levels.outerWick : c1.close > levels.outerWick),
        /* R6 — اكتملت: سكّر السعر خلف مستوى Open قبل الإبطال. */
        R6: completedAtIndex !== null,
      },
...
/**
 * الحالات اللي بتحقق **كل** القواعد المحسومة.
 *
 * ⚠️ هاي مش «فلتر لتحسين النتيجة» — هاي تنفيذ لقرارات محسومة سلفاً.
 * عرض حالة كتلتها ذيل صافي (جسم ٣٪) على صاحب المنهجية بيضيّع وقته على
 * إشي هو نفسه قرر إنه مش كتلة. مقيس على العيّنة: ١٢١ حالة بتخالف R4
 * و٤ بتخالف R5 — يعني ٧ من كل ٢٤ حالة معروضة كانت مخالفة.
 *
 * R1 عمداً **مش** من ضمنها: هو المجهول اللي التسميات بتحدده.
 */
export function onlySettled(cases) {
  return cases.filter((c) => c.settledRules.R3 && c.settledRules.R4 && c.settledRules.R5 && c.settledRules.R6);
}

/**
 * المؤهَّل للتسمية = مطابق للقواعد المحسومة **و** صمد كفاية ليُحكم عليه.
 *
 * ⚠️ الشرط التاني قرار عيّنة مكشوف، مش قاعدة — شوف `LABELING_MIN_SURVIVAL_BARS`.
 * ما بيمس المحرك: هناك R5 قاعدة دورة حياة بلا نافذة، وهيك بتضل.
 */
export function eligibleForLabeling(cases, { minSurvivalBars = LABELING_MIN_SURVIVAL_BARS } = {}) {
  return onlySettled(cases).filter((c) => {
    const s = c.measurements.survivedBars;
    return s == null || s >= minSurvivalBars; // `null` = ما انكسرت أبداً
  });
}
