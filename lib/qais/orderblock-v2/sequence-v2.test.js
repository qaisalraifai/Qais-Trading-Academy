...
test("القيود التلاتة كلها ضرورية — كل واحد بيغيّر الاختيار", () => {
  ...
  /* ⚠️ الفحص بيمشي على المجموعة **بلا** قاعدة 0.382 عمداً.
     -----------------------------------------------------------------
     القاعدة (من وثيقته، القانون ٣) دخلت بعد ما انكتب هالاختبار، وهي
     بترمي المرشّحة اللي كان قيد C بيستبعدها — فبلاها ما بيضل ممكن نثبت
     إنه قيد C إله أثر. القيد **لسا بالكود ولسا صحيح**، بس ضرورته ما
     عادت قابلة للإثبات على هالعيّنة.

     التفصيل بالاختبار اللي بعده: المنافِسة ساقها 430.54 وتصحيحها ٣٢٪
     — تحت 0.382، فقاعدة وثيقته بترميها لحالها وبيطلع سيكونزه الأكبر
     بحجمه وبس. يعني القاعدة **بتعيد إنتاج** نفس الاختيار من طريق تاني. */
  const all = enumerateSequences(GC, GST.majorSwings, at, { minRetracement: 0 });
...
test("⚠️ قاعدة 0.382 بتعيد إنتاج اختياره من طريق مستقل", () => {
  /* ============================================================================
     قبل ما تدخل القاعدة، أكبر سيكونز حيّة بنفس الاتجاه كانت **ساق 430.54**
     — مش سيكونزه. وكان لازم قيد «C مؤكَّدة» عشان تنستبعد.

     تصحيح تلك المرشّحة = **٣٢٪**، تحت الحد المنصوص عليه بوثيقته (0.382).
     فالقاعدة بترميها لحالها، وسيكونزه (261.38) بيصير الأكبر بحجمه وبس.

     يعني قاعدة من وثيقته وقيد استُنتج من مقارنة برسمه اليدوي **بيوصلوا
     لنفس النتيجة** — دليلان مستقلان على نفس الاختيار.
     ============================================================================ */
  const at = GC.length - 1;
  const trend = GST.events[GST.events.length - 1].direction;
  const alive = (opts) =>
    enumerateSequences(GC, GST.majorSwings, at, opts)
      .filter((s) => s.alive && s.direction === trend)
      .sort((a, b) => b.legLength - a.legLength);

  const before = alive({ minRetracement: 0 });
  const after = alive({});

  const rival = before.find((s) => Math.abs(s.legLength - 430.54) < 2);
  assert.ok(rival, "شرط مسبق: المنافِسة موجودة بلا القاعدة");
  assert.ok(rival.retracement < 0.382, `تصحيح المنافِسة ${rival.retracement} — المفروض تحت الحد`);

  assert.ok(Math.abs(before[0].legLength - 430.54) < 2, "بلا القاعدة الأكبر لازم تكون المنافِسة");
  assert.ok(Math.abs(after[0].legLength - 261.38) < 2, `مع القاعدة الأكبر لازم تكون سيكونزه — طلعت ${after[0].legLength}`);
  assert.equal(after.length, before.length - 1, "القاعدة لازم ترمي وحدة بالضبط");
});
