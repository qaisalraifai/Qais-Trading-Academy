...
test("⚠️ مرساة المرآة: بتصلّح الهابط وما بتلمس الصاعد المتحقَّق", () => {
  ...
  const i = at(D1, ts);
  const g = impulseLegAt(swD1, i, true, { anchor: "global" });
  ...
  const gd = impulseLegAt(swD1, i, false, { anchor: "global" });
...
test("الافتراضي `mirror` — بقراره «شغل المرآة»", () => {
  const ts = Date.UTC(2026, 6, 29, 12) / 1000;
  const i = at(D1, ts);
  const def = impulseLegAt(swD1, i, false);
  assert.deepEqual(def, impulseLegAt(swD1, i, false, { anchor: "mirror" }), "الافتراضي مش mirror");

  /* و`global` لسا شغّال للمقارنة — وبيعطي نتيجة مختلفة فعلاً بالهابط. */
  const g = impulseLegAt(swD1, i, false, { anchor: "global" });
  assert.notEqual(g.threshold, def.threshold, "النسختان بتعطيا نفس الشي — الخيار بلا أثر");

  /* الصاعد متطابق بالنسختين — هاد شرط قبول المرآة أصلاً. */
  assert.equal(
    impulseLegAt(swD1, i, true).threshold,
    impulseLegAt(swD1, i, true, { anchor: "global" }).threshold,
    "المرآة غيّرت الثلث الصاعد"
  );
});
