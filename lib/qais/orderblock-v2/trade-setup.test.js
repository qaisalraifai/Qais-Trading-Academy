...
import { buildTradeSetup, SETUP_DEFAULTS } from "./trade-setup.js";
import { CISD_DEFAULTS } from "./cisd.js";
...
test("السلسلة الكاملة على الكتلة المتحقَّقة", () => {
  ...
  assert.ok(r.risk > 0);
  /* ⚠️ الهامش تحت نقطة الـSMT **مبني بس مضاعفه صفر** لحد ما يحدّده صاحب
     المنهجية — قراره (٢٠٢٦-٠٨-٢٠): «لا تعتمد رقماً من حالة واحدة… اعرض لي
     التوزيع قبل تثبيت أي قاعدة».

     فبالإعداد الافتراضي السلوك ما تغيّر: الستوب عند النقطة بالضبط. */
  /* ⚠️ المصدر بينثبّت صراحةً — الافتراضي صار `block` بقراره «شغل حد
     الابطال للستوب» (٢٠٢٦-٠٨-٢١). الاعتماد على الافتراضي بيخلّي الاختبار
     يقارن الشي بنفسه لما يتبدّل. */
  const onSmt = buildTradeSetup(BLOCK, {
    ...baseCtx, correlateLower: { candles: SPX_M15 }, structureOf: SO,
    cisdFrame: { candles: M5, timeframe: "m5" },
  }, { stopSource: "smt", stopBufferAtrMult: 0 });
  assert.equal(onSmt.stop, onSmt.chain.smt.point, "بمصدر smt وهامش صفر الستوب لازم يساوي نقطة الـSMT");
  /* ⚠️ الافتراضي رجع `smt` (٢٠٢٦-٠٨-٢١) بعد ما شرح إنه الستوب «تحت القاع
     يلي ما نزل النازداك تحته» — يعني تحت نقطة الـSMT بتعريفها الجديد. */
  /* ⚠️ الهامش صار 0.25× ATR (٢٠٢٦-٠٨-٢١) — فالستوب **تحت** النقطة مش
     عندها. الفحص على الاتجاه ووجود الفرق، مش على المساواة. */
  const upR = r.direction === "up";
  assert.ok(
    upR ? r.stop < r.chain.smt.point : r.stop > r.chain.smt.point,
    `الستوب ${r.stop} مش ${upR ? "تحت" : "فوق"} نقطة الـSMT ${r.chain.smt.point}`
  );
  assert.ok(Math.abs(r.stop - r.chain.smt.point) > 0, "الهامش ما انطبّق");

  /* والآلية شغّالة: بمضاعف صريح بيزحّط بالاتجاه الصح بمقدار ATR × المضاعف. */
  const up = r.direction === "up";
  const withBuffer = buildTradeSetup(BLOCK, {
    ...baseCtx, correlateLower: { candles: SPX_M15 }, structureOf: SO,
    cisdFrame: { candles: M5, timeframe: "m5" },
  }, { stopBufferAtrMult: 0.5 });
  assert.equal(withBuffer.ok, true);
  assert.ok(
    up ? withBuffer.stop < withBuffer.chain.smt.point : withBuffer.stop > withBuffer.chain.smt.point,
    `بهامش 0.5× الستوب ${withBuffer.stop} مش ${up ? "تحت" : "فوق"} نقطة الـSMT ${withBuffer.chain.smt.point}`
  );
  assert.ok(withBuffer.risk > r.risk, "الهامش لازم يوسّع المخاطرة مش يضيّقها");
...
test("مصدر الستوب: الافتراضي `smt` (القاع الصامد)، و`block` متاح", () => {
  /* ⚠️ `stopSource: "block"` بيحط الستوب عند **حد إبطال الكتلة** — وهو
     مستوى من منهجيته كمان (R5: «بتنلغى لما يسكّر السعر خلف آخر مستوى
     فيها»)، مش مستوى مخترع.

     القياس على الحالتين المكتملتين: نقطة الـSMT انضربت بالاتنتين، وحد
     الإبطال نجا بالاتنتين — بس بمخاطرة ٧–١٠× ATR. N = حادثة سوق وحدة،
     فالقرار معلّق. الافتراضي `smt` كما نطقه. */
  const base = { ...baseCtx, correlateLower: { candles: SPX_M15 }, structureOf: SO, cisdFrame: { candles: M5, timeframe: "m5" } };
  /* ⚠️ الهامش مثبَّت صفر هون عشان تنعزل المقارنة على **المصدر** وحده —
     الافتراضي صار 0.25× ATR وبيزحّط الاتنين بنفس المقدار. */
  const smt = buildTradeSetup(BLOCK, base, { stopBufferAtrMult: 0 });
  assert.equal(smt.ok, true, smt.reason || smt.why);
  assert.equal(smt.stop, smt.chain.smt.point, "بمصدر افتراضي وهامش صفر الستوب = نقطة الـSMT");
  assert.equal(smt.stopLabel, "نقطة الـSMT");

  const blk = buildTradeSetup(BLOCK, base, { stopSource: "block", stopBufferAtrMult: 0 });
  assert.equal(blk.ok, true, blk.reason || blk.why);
  const level = BLOCK.levels.invalidation ?? BLOCK.levels.outerWick;
  assert.equal(blk.stop, level, "بـstopSource=block الستوب لازم يكون حد الإبطال");
  assert.equal(blk.stopLabel, "حد إبطال الكتلة", "التسمية لازم تطلع بالمخرج — الشارت بيقراها");
  assert.ok(blk.risk > smt.risk, "حد الإبطال أبعد من القاع الصامد، فالمخاطرة أوسع");

  /* ⚠️ الدخول **ممكن يختلف** بين المصدرين — وهاد سلوك صحيح بعد إعادة
     ترتيب السلسلة (SMT ثم CISD بعدها).

     السبب: بوابة الصلاحية «الستوب بالجهة الصح من الدخول» بتعتمد على
     الستوب نفسه. بمصدر `block` الستوب ثابت (حد الإبطال)، فأول SMT إلها
     CISD فوقه بتمشي. وبمصدر `smt` الستوب بيتغيّر مع كل مرشّحة، فممكن
     تنقبل مرشّحة تانية. قبل إعادة الترتيب كان الدخول واحداً دايماً لأن
     الـCISD كانت بتنحسب مرة وحدة من أول SMT.

     اللي **لازم** يضل صحيحاً: الاتنان يعطوا إعداداً متماسكاً. */
  for (const r of [smt, blk]) {
    const up = r.direction === "up";
    assert.ok(up ? r.stop < r.entry : r.stop > r.entry, `ستوب ${r.stop} بالجهة الغلط من ${r.entry}`);
    assert.ok(r.chain.smt.time <= r.chain.cisd.time, "الـCISD قبل الـSMT — ترتيب مكسور");
  }
});

/* ============================================================================
   ⚠️ حارس: كل خيار بينقرا من `cfg` لازم يكون **معلناً** بمكان ما.

   انبنى بعد ما `smtSearchBarsLower` قعد **سطرياً** جوّا التعبير
   (`cfg.smtSearchBarsLower ?? 48`) — مش بالإعدادات، بلا تعليق وبلا مرجع.
   وطلع بالقياس إنه **هو** الرقم اللي بيحدّد عدد الصفقات (٤٨→٦ · ٢٤→٣ ·
   ٦→٠)، بينما `smtSearchBars` المعلن والموثَّق **بلا أثر** بالإعداد
   القائم (١ و١٢ و٥٠٠ بيعطوا نفس النتيجة).

   الرقم اللي بيحكم النتيجة كان مخفي، والمرئي كان ميت. الحارس بيمنع
   الرجوع لهناك: أي `cfg.X` بالكود لازم يكون بـ`SETUP_DEFAULTS` (خيار
   إلنا) أو `CISD_DEFAULTS` (خيار بينتمرّر لـ`findCISD`).
   ============================================================================ */
test("ولا خيار بينقرا من cfg بلا ما يكون معلناً", () => {
  const src = fs.readFileSync(path.join(HERE, "trade-setup.js"), "utf8");
  const read = new Set([...src.matchAll(/\bcfg\.([A-Za-z_]\w*)/g)].map((m) => m[1]));
  assert.ok(read.size >= 5, `قرا ${read.size} خيار بس — الحارس شبه فاضي`);

  const declared = new Set([...Object.keys(SETUP_DEFAULTS), ...Object.keys(CISD_DEFAULTS)]);
  const undeclaredOpts = [...read].filter((k) => !declared.has(k));
  assert.deepEqual(
    undeclaredOpts, [],
    "خيارات بتنقرا من cfg بلا إعلان — قيمتها الفعلية مخفية بالكود:\n  " + undeclaredOpts.join("\n  ")
  );
});

test("نافذة الـSMT = ١٢ ساعة — قراره، مثبَّتة", () => {
  /* ✅ قراره (٢٠٢٦-٠٨-٢٢): انعرضت عليه الحالة الملموسة «السعر لمس الكتلة
     ١٢ الظهر، وصار SMT ١١ بالليل — بعد ١١ ساعة. بتقبلها؟» وجوابه
     **«بقبلها»**. فالنافذة لازم تسع ١١ ساعة على الأقل.

     ⚠️ الاختبار بيفحص **المدة الزمنية** مش عدد الشموع — لو تبدّل فريم
     البحث، ٤٨ شمعة بتصير مدة تانية والقرار بينكسر بصمت. هاد بالضبط
     الصنف اللي انزاح فيه مصدر الستوب من غير ما ينتبه حدا. */
  const SMT_TF_MINUTES = 15;
  const ACCEPTED_HOURS = 11;
  const hours = (SETUP_DEFAULTS.smtSearchBarsLower * SMT_TF_MINUTES) / 60;
  assert.equal(hours, 12, `النافذة ${hours} ساعة — قراره ١٢`);
  assert.ok(hours >= ACCEPTED_HOURS, `النافذة ${hours} ساعة بترفض حالة قبلها هو (${ACCEPTED_HOURS} ساعة)`);
});

test("باقي النوافذ مثبَّتة — تغييرها قرار مش مصادفة", () => {
  /* ⚠️ مش تعييراً: مثبَّتة عشان أي تعديل يبان بالـdiff ويتناقش.
     `smtSearchBars` ميت بالإعداد القائم، و`cisdMaxBars` بلا مرجع. */
  assert.equal(SETUP_DEFAULTS.smtSearchBars, 12);
  assert.equal(SETUP_DEFAULTS.cisdMaxBars, 60);
});
