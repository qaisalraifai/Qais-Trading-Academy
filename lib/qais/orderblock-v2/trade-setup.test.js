...
test("السلسلة الكاملة على الكتلة المتحقَّقة", () => {
  ...
  assert.equal(r.stop, BLOCK.levels.invalidation ?? BLOCK.levels.outerWick, "الافتراضي لازم يكون حد الإبطال");
...
test("مصدر الستوب: الافتراضي `block` بقراره، و`smt` متاح للمقارنة", () => {
  ...
  const smt = buildTradeSetup(BLOCK, base, { stopSource: "smt" });
  assert.equal(smt.ok, true, smt.reason || smt.why);
  assert.equal(smt.stop, smt.chain.smt.point, "بمصدر smt الستوب لازم يكون نقطة الـSMT");
  assert.equal(smt.stopLabel, "نقطة الـSMT");

  const blk = buildTradeSetup(BLOCK, base);
  assert.equal(blk.ok, true, blk.reason || blk.why);
  const level = BLOCK.levels.invalidation ?? BLOCK.levels.outerWick;
  assert.equal(blk.stop, level, "الافتراضي لازم يكون حد الإبطال");
  assert.equal(blk.stopLabel, "حد إبطال الكتلة", "التسمية لازم تطلع بالمخرج — الشارت بيقراها");
  assert.ok(blk.risk > smt.risk, "حد الإبطال أبعد من نقطة الـSMT، فالمخاطرة أوسع");

  /* الدخول ما بيتغيّر — المصدر بيمسّ الستوب وبس. */
  assert.equal(blk.entry, smt.entry);
});
