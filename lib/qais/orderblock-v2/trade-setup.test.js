...
test("السلسلة الكاملة على الكتلة المتحقَّقة", () => {
  ...
  assert.equal(r.stop, r.chain.smt.point, "الافتراضي لازم يكون نقطة الـSMT (القاع الصامد)");
...
test("مصدر الستوب: الافتراضي `smt` (القاع الصامد)، و`block` متاح", () => {
  ...
  const smt = buildTradeSetup(BLOCK, base);
  assert.equal(smt.ok, true, smt.reason || smt.why);
  assert.equal(smt.stop, smt.chain.smt.point, "الافتراضي لازم يكون نقطة الـSMT");
  assert.equal(smt.stopLabel, "نقطة الـSMT");

  const blk = buildTradeSetup(BLOCK, base, { stopSource: "block" });
