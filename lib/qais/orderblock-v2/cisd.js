/* ============================================================================
   ...
   ⚠️ «قمة» = **الذيل** (`high`) مش أعلى الجسم — قرار صريح (٢٠٢٦-٠٨-١٨).
   بشمعة هابطة أعلى الجسم هو الفتح، والذيل أعلى منه. الفرق عملي: بالذيل
   المستوى أعلى، فالدخول متأخر وبتأكيد أقوى.
...
export const CISD_DEFAULTS = {
  /* ⚠️ شرط الملاصقة — مش من نص القاعدة، انضاف بالتنفيذ.
     نصّه (٢٠٢٦-٠٨-١٩): «CISD يعني كسر قمة آخر شمعة معاكسة على فريم ١٥
     دقيقة أو ٥ دقائق». ما ذكر إنه الشمعة الكاسرة لازم تكون **ملاصقة**
     للسلسلة.

     ✅ **انشال** (٢٠٢٦-٠٨-٢١، قراره: «شغل … شرط الملاصقة»). الافتراضي
     صار `false` — يعني أي شمعة باتجاه الصفقة بتكسر طرف آخر سلسلة معاكسة
     قبلها بتعطي CISD، بلا اشتراط إنها ملاصقة إلها.
     `true` انترك للمقارنة وللاختبار اللي بيوثّق الفرق. */
  requireAdjacentRun: false,
 ...
 */
export function lastOppositeRun(candles, beforeIndex, dirUp, requireAdjacent = false) {
  /* ⚠️ `requireAdjacent` بيحدّد هل السلسلة لازم تكون **ملاصقة** للشمعة
     الكاسرة، ولا أي سلسلة معاكسة قبلها.

     الفرق مقيس ومهم: بالملاصقة، أول شمعة صاعدة بعد سلسلة هابطة لو ما
     كسرت طرف السلسلة كلها، الشموع اللي بعدها **بتنتخطّى** لأنها ما عاد
     قبلها سلسلة ملاصقة — فالمحرك بيستنى سلسلة معاكسة جديدة، وهي بتتكوّن
     أعلى، فالدخول بيطلع أبعد عن نقطة الكنس.

     مقيس على الكتلة المتحقَّقة (٢٩ يوليو): قاع الكنس 27,268.30 عند 16:15،
     والدخول طلع 27,592.83 عند 17:10 — **٣٢٤ نقطة (3.24× ATR) فوق الكنس**،
     بينما أول كسر فعلي كان 27,374.17 عند 16:20 (1.06× ATR).

     ✅ الافتراضي صار `false` بقراره (٢٠٢٦-٠٨-٢١). */
  let start = beforeIndex - 1;
  if (!requireAdjacent) {
    /* نتخطّى الشموع اللي بنفس الاتجاه لحد ما نلاقي آخر سلسلة معاكسة. */
    while (start >= 0 && !(dirUp ? candles[start].close < candles[start].open : candles[start].close > candles[start].open)) start--;
  }
  const run = [];
...
export function findCISD(candles, fromIndex, dirUp, options = {}) {
  ...
  for (let i = start; i <= limit; i++) {
    ...
    const run = lastOppositeRun(candles, i, dirUp, cfg.requireAdjacentRun);
    if (!run) continue;
