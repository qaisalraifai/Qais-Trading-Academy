...
test("المستوى المرجعي = أعلى قمة بالسلسلة كلها مش آخر شمعة", () => {
  ...
  /* ⚠️ «قمة» = **الذيل** مش أعلى الجسم — قرار صريح (٢٠٢٦-٠٨-١٨).
     بشمعة هابطة أعلى الجسم هو الفتح (99.5 هون) والذيل 105. الفرق عملي:
     بالذيل المستوى أعلى فالدخول متأخر وبتأكيد أقوى. */
  const bodyTop = Math.max(...C.slice(1).map((c) => Math.max(c.open, c.close)));
  assert.ok(run.level > bodyTop, `المستوى ${run.level} = أعلى جسم مش ذيل`);
  assert.equal(run.level, Math.max(...C.slice(1).map((c) => c.high)), "المستوى مش أعلى ذيل بالسلسلة");
});
