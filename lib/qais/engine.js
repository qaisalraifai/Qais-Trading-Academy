...
export function analyzeSymbol({ symbol, candlesByTF, correlated = null, previousState = null, newsBlocked = null }) {
  ...
  /* كتل الأوامر على **فريم عرض الصفقة نفسه**.
     قبل هيك كنا منمرّر htfOrderBlocks (محسوبة على mainCandles) ومنرجع
     لنموذج التصحيح لو الفريمين مختلفين — يعني الدخول من الـOB كان بينطفي
     بصمت. هلأ منحسبها للفريم المطلوب مباشرة (ومنعيد استخدام حساب
     الفريم الرئيسي لو صادف إنهم نفس الفريم، تفادياً لحساب مكرر). */
  /* سياق الـSMT التاريخي: أحداث الهيكلية للأصلين على **نفس فريم عرض
     الصفقة**، مع سماحية تزامن = 10 شمعات (نفس رقم analyzeSMT الحي، بس
     محوّل لثواني لأن فهارس أصلين ما بتتطابق لو في فجوات تداول مختلفة). */
  const lastTradeDebug = {}; // تشخيص مؤقت — لمعرفة ليش ما بتطلع صفقة
  const obScanDebug = {}; // تشخيص مؤقت — مسح كتل الأوامر
  let lastTradeSmtCtx = null;
  if (correlated?.candlesByTF?.[lastTradeTF]?.length >= 30 && lastTradeCandles?.length >= 2) {
    const corrStruct = analyzeStructure(correlated.candlesByTF[lastTradeTF]);
    const barSec = Math.abs(lastTradeCandles[1].time - lastTradeCandles[0].time) || 3600;
    lastTradeSmtCtx = {
      eventsA: structByTF[lastTradeTF]?.events || [],
      eventsB: corrStruct?.events || [],
      toleranceSec: barSec * 10,
    };
  }

  /* كتل الصفقة الأخيرة — لازم تنحسب بـ keepInvalid حتى نقدر نقيّم صلاحية
     الكتلة **بلحظة الدخول** بدل لحظة اليوم. وبتنحسب بنفس الطريقة سواء كان
     فريم العرض هو الفريم الرئيسي أو لأ (قبل هيك كان فرع mainTF بيعيد استخدام
     htfOrderBlocks، فما كان يمرّ على هالمسار ولا يعبّي التشخيص). */
  let lastTradeObZones = null;
  if (lastTradeCandles?.length && structByTF[lastTradeTF]) {
    const tfStruct = structByTF[lastTradeTF];
    const tfLiquidity =
      lastTradeTF === mainTF ? liquidity : analyzeLiquidity(lastTradeCandles, tfStruct);
    const tfLegs = lastTradeTF === mainTF ? mssLegs : getAllMSSLegs(lastTradeCandles, tfStruct);
    obScanDebug.legs = tfLegs.map((l) => ({
      from: l.fromIndex,
      to: l.toIndex,
      dir: l.direction,
      obs: scanLegOrderBlocks(lastTradeCandles, l, tfLiquidity.fvgs, { keepInvalid: true }).length,
      priceFrom: lastTradeCandles[l.fromIndex]?.close ?? null,
      priceTo: lastTradeCandles[Math.min(l.toIndex, lastTradeCandles.length - 1)]?.close ?? null,
    }));
    obScanDebug.fvgCount = tfLiquidity.fvgs?.length ?? 0;
    obScanDebug.candleCount = lastTradeCandles.length;
    lastTradeObZones = tfLegs.length
      ? scanAllLegsOrderBlocks(lastTradeCandles, tfLegs, tfLiquidity.fvgs, { keepInvalid: true })
      : tfLiquidity.window
        ? scanLegOrderBlocks(lastTradeCandles, tfLiquidity.window, tfLiquidity.fvgs, { keepInvalid: true })
        : [];
  }
  const lastTrade =
    lastTradeCandles?.length && structByTF[lastTradeTF]
      ? findLastTrade(
          lastTradeCandles,
          structByTF[lastTradeTF],
          trend,
          {
            h4Mss: structByTF.h4?.lastMSS?.level ?? null,
            dailyBos: structByTF.daily?.lastBOS?.level ?? null,
          },
          /* كتل الأوامر — محسوبة على mainCandles، فما بتنفع إلا لو فريم عرض
             الصفقة هو نفسه الفريم الرئيسي. غير هيك منمرّر null ومنرجع
             لنموذج التصحيح — أفضل من مطابقة أوقات بين فريمين مختلفين. */
          lastTradeObZones,
          lastTradeSmtCtx,
          lastTradeDebug,
          /* قيد الساق: من بداية السيكونز المعروضة نفسها — هيك الصفقة
             مضمون إنها من نفس الساق يلي شايف نقاطها قدّامك.
             قبل هيك كنت مفلتر "بعد آخر MSS" — ولو صار MSS بعد آخر BOS
             بيطلع صفر مرشّحين، مع إنه السيكونز المرسومة مبنية على ذاك الـBOS. */
          sequence?.points?.origin?.index ?? null
        )
      : null;
  ...
  /* ============================================================================
     جودة البيانات: هل شموع الفريمات متّسقة مع بعضها؟
     ----------------------------------------------------------------------------
     سلسلة المزوّدين بتتراجع بصمت لما يفشل الأساسي، وممكن تخلط سلسلتين من
     عقود مختلفة. قياس فعلي على الذهب: ١.٦٪ بس من الشموع اليومية بتحتوي
     شموعها الأربع-ساعية — الشمعة اليومية ٢٠٢٦-٠٨-١٣ مداها 4350–4445
     بينما تجميع شموع الـH4 تبعها 4400–4509.1.

     كل التحليل تحت (هيكل، سيولة، كتل، متعدد الفريمات) بيفترض إنه الفريمات
     بتوصف نفس السوق. لو مش هيك، النتيجة مبنية على تناقض — فبينكشف بدل
     ما يمرق. ما منوقف التحليل: القرار للواجهة والمستخدم. */
  const dataQuality = checkTimeframeConsistency(candlesByTF);

  /* ── سلسلة QAIS SK الجديدة — **تشغيل موازٍ** ────────────────────────
     كل الحقول فوق من المحركات القديمة، والواجهة بتقرا منها بآلاف الأسطر.
     استبدالها دفعة وحدة بيغيّر كل اللي بيشوفه المستخدمون بضربة وبلا مرجع
     للمقارنة. فالسلسلة الجديدة بتنضاف **جنبها** بحقل مستقل، وبينقارنوا
     على بيانات حقيقية، وبعدها بينقرر التبديل.

     ⚠️ `runSkV2` ما بترمي أبداً — أي نقص بيرجع INSUFFICIENT_DATA مع سبب.
     كسر التحليل الحيّ لكل الرموز بسبب طبقة تجريبية مش مقبول. */
  const skV2 = runSkV2({
    symbol,
    candlesByTF,
  ...
  return {
    ...
    lastTradeDebug: { ...lastTradeDebug, timeframe: lastTradeTF, correlated: correlated?.symbol || null }, // مؤقت
    obScanDebug, // مؤقت
    /* كتلة الأوامر والـSMT — محسوبين على فريم التنفيذ (m5/m15)، فما منقدر
       نربطهم بزمن شموع فريم العرض. منصدّرهم كمستويات سعرية وبس —
       والواجهة بترسمهم كنطاقات أفقية (نفس أسلوب OB+/OB- بتريدنغ فيو). */
    /* كل كتل الأوامر الصالحة على الفريم الرئيسي (ذاكرة الـOB) — مش بس الوحدة
       النشطة. هدول اللي بترسمهم الواجهة كخطوط OB+/OB- أفقية. */
    orderBlocks: (htfOrderBlocks || []).map((o) => ({
      direction: o.direction,
      status: o.status,
      quality: o.quality,
      high: o.merged?.high ?? null,
      low: o.merged?.low ?? null,
      mt: o.levels?.mt ?? null,
      invalidation: o.levels?.invalidation ?? null,
      /* مستويات الكتلة كاملة — الواجهة بترسم كل واحد كخط مستقل */
      levels: o.levels
        ? { mt: o.levels.mt, open: o.levels.open, close: o.levels.close, fvg: o.levels.fvg, outerWick: o.levels.outerWick }
        : null,
      strengthOrder: o.strengthOrder || null,
      time: o.time ?? null,
    })),
    /* مرجع TP1: إما MSS على 4H أو BOS على اليومي — حسب تعريف المنهجية،
       مش أي سوينغ قريب (كان يطلع "قمة" على شمعة ما إلها معنى هيكلي). */
    peakRefs: {
      h4Mss: structByTF.h4?.lastMSS?.level ?? null,
      dailyBos: structByTF.daily?.lastBOS?.level ?? null,
    },
    orderBlock: ob?.eligible
      ? {
          direction: ob.direction,
          status: ob.status,
          quality: ob.quality,
          high: ob.merged?.high ?? null,
          low: ob.merged?.low ?? null,
          levels: ob.levels || null,
          timeframe: obTF || null,
        }
      : null,
    smtSignal: smt?.valid
      ? {
          point: smt.point,
          pointType: smt.pointType,
          strength: smt.strength,
          correlatedWith: smt.symbolB || null,
          timeframe: smt.timeframe || null,
        }
      : null,
    context: {
