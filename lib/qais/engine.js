...
export function analyzeSymbol({ symbol, candlesByTF, correlated = null, previousState = null, newsBlocked = null }) {
  ...
  if (lastTradeCandles?.length && structByTF[lastTradeTF]) {
    if (lastTradeTF === mainTF) {
      lastTradeObZones = htfOrderBlocks;
    } else {
      const tfStruct = structByTF[lastTradeTF];
      const tfLiquidity = analyzeLiquidity(lastTradeCandles, tfStruct);
      const tfLegs = getAllMSSLegs(lastTradeCandles, tfStruct);
      obScanDebug.legs = tfLegs.map((l) => ({
        from: l.fromIndex,
        to: l.toIndex,
        dir: l.direction,
        obs: scanLegOrderBlocks(lastTradeCandles, l, tfLiquidity.fvgs).length,
      ...
      lastTradeObZones = tfLegs.length
        ? scanAllLegsOrderBlocks(lastTradeCandles, tfLegs, tfLiquidity.fvgs)
        : tfLiquidity.window
          ? scanLegOrderBlocks(lastTradeCandles, tfLiquidity.window, tfLiquidity.fvgs)
          : [];
    }
  }
  const lastTrade =
