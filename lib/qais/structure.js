...
function tryBOS(direction, s0, s1, s2, candles, requireBeyondLevel = null) {
  if (direction === "up") {
    ...
    // ratio لازم يكون بين 0.333 و1 — لو تجاوز 1 يعني s2 كسر تحت s0 بالكامل،
    ...
    if (!brk) return null;
    return buildBOSEvent("up", s0, s1, s2, brk, candles);
  }
  ...
  if (!brk) return null;
  return buildBOSEvent("down", s0, s1, s2, brk, candles);
}
...
export function analyzeStructure(candles, { lookback = 2 } = {}) {
  ...
  while (i < swings.length) {
    ...
    if (mssBrk) {
      ...
      // منطلق البحث عن الساق الجديدة = نفس السوينغ يلي عرّف الـ protectedLevel
      // المكسور (هو فعلياً نقطة الانعكاس/الـ Origin تبع الساق الجديدة)
      const anchorIdx = swings.findIndex((s) => s.index === protectedLevel.index);
      i = anchorIdx >= 0 ? anchorIdx + 2 : i + 1;
      protectedLevel = null;
      continue;
    }

    // ما في استمرار ولا MSS بعد — هاي آخر حالة حاكمة معروفة، منوقف
    break;
  }
...
export function getAllMSSLegs(candles, structureResult) {
  ...
  return mssEvents.map((mss, idx) => ({
    fromIndex: mss.index,
    toIndex: idx + 1 < mssEvents.length ? mssEvents[idx + 1].index - 1 : candles.length - 1,
