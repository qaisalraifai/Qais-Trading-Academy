...
import { SWEEP_DEFAULTS, scanPoolInteractions, resolveEpisode, buildSweep, mergeConfluentSweeps, dedupeWickBreaks } from "./sweeps.js";
...
export function analyzeLiquidityV2(candles, options = {}) {
  ...
  */
  ...
  /* ⚠️ لمسة سعرية وحدة = حدث واحد، حتى لو التقت عليها كذا بركة.
     `pool.sweeps` بتضل خام (كل بركة انكنست فعلاً)، بس القائمة العليا
     بتمثّل **أحداث السوق** — وإلا كل إحصاء سيولة بينتضخّم. */
  const merged = mergeConfluentSweeps(poolInteractions);
  const sweeps = merged.sweeps;
  ...
  const metrics = {
    ...
    sweeps: {
      total: sweeps.length,
      /* ⚠️ الرقمان مختلفان عن قصد: `total` أحداث سوق، و`poolInteractions`
         كم بركة انكنست. الفرق = التقاء مستويات، وهو معلومة مش ضجيج. */
      poolInteractions: poolInteractions.length,
      mergedByConfluence: merged.mergedCount,
      outcomeConflicts: merged.conflicts,
      confluent: sweeps.filter((s) => s.confluence.poolCount > 1).length,
      maxConfluence: sweeps.reduce((m, s) => Math.max(m, s.confluence.poolCount), 0),
