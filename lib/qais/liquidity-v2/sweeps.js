...
function buildStructuralMeaning({ pool, outcome, disp, contDisp, alignedEvent, opposedEvent, reactionBars }) {
  ...
  return {
    code,
    /* `reversed` = انعكاس **هيكلي**، مش مجرد ابتعاد سعري. لهيك بينفى لما
       يكون الحدث الوحيد بالنافذة بنفس اتجاه الانسحاب. */
    reversed: outcome === "reversal" && code !== "SWEPT_REVERSED_THEN_STRUCTURE_CONTINUED",
...
export function buildSweep(candles, pool, episode, resolved, options = {}) {
  ...
  return {
    ...
    pool: { id: pool.id, type: pool.type, price: pool.price, side: pool.side, strength: pool.strength },
