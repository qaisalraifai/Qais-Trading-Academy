...
test("atrAtCausal ما بياخد قيمة من المستقبل (بعكس atrAt بمحرك الهيكل)", () => {
  const s = [null, null, null, 5, 7];
  /* الضابط: نسخة الهيكل بترجّع ٥ للفهرس ٠ — قيمة من الفهرس ٣، يعني من
     المستقبل. بنثبّت هالسلوك بالاختبار حتى الفرق يكون موثّق مش ضمني. */
  assert.equal(structureAtrAt(s, 0), 5, "سلوك atrAt المعروف: بيمسح للأمام");
  assert.equal(atrAtCausal(s, 0), null, "النسخة السببية لازم ترجّع null مش قيمة مستقبلية");
