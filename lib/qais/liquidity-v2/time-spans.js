...
export function buildTimeSpans(candles, options = {}) {
  ...
  /* ---- أيام ----
     ...
     ✅ **محسوم بقرار صاحب المنهجية: الشمعة اليومية تبع المزوّد.**
     فلما تنمرّر شموع يومية، هي المعتمدة. التراجع لليوم التداولي المستنتَج
     بيصير **بس** لما ما تتوفّر شموع يومية إطلاقاً — وبينتوسم بـdefinition
     مختلف حتى ما ينخلط بالقراءة.

     ⚠️ يعني تمرير `dailyCandles` مش اختياري عملياً: بدونه المستويات
     بتتحسب بتعريف تاني بيعطي مستويات مختلفة كلياً (١٩ بِركة متطابقة بس
     من أصل ~١٥٤ بالقياس).
  */
  let day;
