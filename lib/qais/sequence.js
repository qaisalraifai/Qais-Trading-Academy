...
function buildTradeFromBOS(candles, bos, swings, peakRefs) {
  ...
  const entry = findEntryTouch(candles, C.index, wcl, direction);
  ...
  for (const t of targets) {
    t.hit = false;
    for (let i = entry.index + 1; i <= scanEnd; i++) {
      const c = candles[i];
      if (!c) continue;
 ...
 */
export function findLastTrade(candles, structureResult, expectedDirection = null, peakRefs = null) {
  const { events, swings } = structureResult || {};
  ...
  for (let i = bosEvents.length - 1; i >= 0; i--) {
    const trade = buildTradeFromBOS(candles, bosEvents[i], swings, peakRefs);
