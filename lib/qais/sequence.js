/* ============================================================================
   ...
   Origin→A، مسقطة من C (نفس منطق Extension بتريدنغ فيو حرفياً):
...
function findConfirmedContinuation(swings, fromIndex, pivotType, extremePrice, direction) {
...
export function analyzeSequence(candles, structureResult) {
  ...
  const C = findConfirmedContinuation(swings, lastBOS.index, pivotType, A.price, direction);

  // -------- إلغاء قبل ما C تتأكد أصلاً: رجع السعر وكسر B بالاتجاه المعاكس --------
  ...
  // -------- إلغاء بعد تأكيد C: رجع السعر وكسر B بالاتجاه المعاكس --------
...
export function resolveSequence(sequencesByTF, priorityOrder = ["h4", "daily", "execution"]) {
  ...
  // ما تختفي الصفحة كلياً لحد ما ينضج السيكونز بالكامل.
  for (const tf of priorityOrder) {
    const seq = sequencesByTF[tf];
    if (seq?.stage === "awaiting-c") return { ...seq, sourceTF: tf };
  }
  return { active: false, sourceTF: null, stage: "none", reason: "لا يوجد سيكونز فعّال أو قيد التكوين على أي فريم من فريمات الأولوية" };
}
