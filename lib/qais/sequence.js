...
function buildTradeFromBOS(candles, bos, swings) {
  ...
  /* TP1 = أقرب قمة/قاع هيكلي سابق قدّام الدخول */
  const priorSwings = (swings || []).filter((s) => s.index <= entry.index);
  const real = nearestRealTarget(priorSwings, direction, entry.price);
  const targets = real
    ? [{ key: "TP1", ratio: null, color: "أخضر", price: real.price, isRealLevel: true }, ...ratioTargets]
    : ratioTargets;
 ...
 */
export function findLastTrade(candles, structureResult, expectedDirection = null) {
  const { events, swings } = structureResult || {};
  ...
  for (let i = bosEvents.length - 1; i >= 0; i--) {
    const trade = buildTradeFromBOS(candles, bosEvents[i], swings);
