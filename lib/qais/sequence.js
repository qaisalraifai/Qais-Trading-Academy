...
/* أول شمعة بعد C لمست نطاق التصحيح (منطقة الدخول) */
function findEntryTouch(candles, fromIndex, wcl, direction) {
  for (let i = fromIndex + 1; i < candles.length; i++) {
    const c = candles[i];
    if (!c) continue;
    // بالصعود التصحيح بينزل للنطاق، فمنفحص القاع؛ وبالهبوط منفحص القمة
    const probe = direction === "up" ? c.low : c.high;
    if (probe >= wcl.low && probe <= wcl.high) {
      // سعر الدخول = حافة النطاق يلي لمسها السعر أول (مش القاع/القمة الكاملة)
      const price = direction === "up" ? Math.min(wcl.high, Math.max(wcl.low, probe)) : Math.max(wcl.low, Math.min(wcl.high, probe));
      return { index: i, time: c.time, price };
    }
  }
  return null;
}
...
function buildTradeFromBOS(candles, bos, swings, peakRefs, obZones, timeToIndex, smtCtx) {
  ...
  const legLength = Math.abs(A.price - origin.price);
  if (legLength <= MIN_LEG) return null;
  ...
  const obEntry = findObEntry(candles, obZones, direction, origin.index, timeToIndex);
  const entry = obEntry || findEntryTouch(candles, C.index, wcl, direction);
  if (!entry) return null;
  const entrySource = obEntry ? "orderBlock" : "retracement";
  ...
  if (smtCtx?.eventsA?.length && smtCtx?.eventsB?.length) {
    ...
    if (!hit) return null; // ما صار تباعد بعد لمسة الـOB — ما اكتملت شروط الدخول
 ...
 */
export function findLastTrade(candles, structureResult, expectedDirection = null, peakRefs = null, obZones = null, smtCtx = null) {
  const { events, swings } = structureResult || {};
  ...
  for (let i = bosEvents.length - 1; i >= 0; i--) {
    const trade = buildTradeFromBOS(candles, bosEvents[i], swings, peakRefs, obZones, timeToIndex, smtCtx);
    if (!trade) continue;
