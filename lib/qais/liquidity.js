...
/* -------------------- تجميع كل مناطق السيولة الستة + فحص اللمس --------------------
   window (خامساً / رابع عشر): { fromIndex, toIndex } — لو انمرر، منقصر البحث فقط
   على المناطق الواقعة ضمن آخر حركة هيكلية رئيسية (بين آخر MSS وآخر BOS، أو ساق
   آخر BOS لو ما في MSS بعد). من غير هيك تحديد، الأداة كانت "تعرض كل شي عشوائياً"
   وهذا بالضبط اللي طلب المستخدم نتجنبه (أولاً). */
export function analyzeLiquidity(candles, structureResult, window = null) {
  ...
  let allZones = [...fvgs, ...voids, ...sweeps, ...brkr, ...mtg, ...rjb].sort((a, b) => a.index - b.index);

  // تقييد النطاق: هامش بسيط (2 شمعة) قبل/بعد حدود الحركة عشان ما نفوّت منطقة
  // تشكّلت بالضبط عند حافة الـ BOS/MSS بسبب فرق الفهرسة بين أنواع المناطق.
  if (window && Number.isFinite(window.fromIndex) && Number.isFinite(window.toIndex)) {
    const lo = Math.min(window.fromIndex, window.toIndex) - 2;
    const hi = Math.max(window.fromIndex, window.toIndex) + 2;
    allZones = allZones.filter((z) => z.index >= lo && z.index <= hi);
  }

  const lastPrice = candles[candles.length - 1]?.close;
  const lastIndex = candles.length - 1;

  // هل السعر الحالي لامس إحدى المناطق الست ضمن الحركة الرئيسية؟ (شرط أساسي قبل
  // البحث عن OB — 2 ملاحظة). بندور من الأحدث للأقدم عشان نلقط آخر فرصة فعلية.
  ...
  // ترتيب المناطق من الأهم للأقل (سادساً): أولوية لأنواع POI الأقوى ثم الأحدث
  const TYPE_RANK = { Sweep: 6, MTG: 5, BRKR: 5, Void: 4, FVG: 3, RJB: 2 };
  const ranked = [...allZones].sort((a, b) => (TYPE_RANK[b.type] || 0) - (TYPE_RANK[a.type] || 0) || b.index - a.index);

  return { fvgs, voids, sweeps, brkr, mtg, rjb, allZones, rankedZones: ranked, touchedZone: touched || null, window };
}
