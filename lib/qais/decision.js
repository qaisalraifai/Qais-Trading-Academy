/* ============================================================================
   ...
   المدخل ctx (يجمّعه lib/qais/engine.js من كل الفريمات):
   {
     ...
     newsBlocked,       // من lib/economic-calendar.getActiveNewsBlock() أو null (الفصل ٩)
   }
...
export function makeDecision(candles, ctx) {
  const { trend, priceLocation, liquidity, ob, sequence, smt, wasActiveBefore, newsBlocked, nearestPeak, biggestSequence } = ctx;
  ...
  const checks = {
    ...
    smtPresent: !!smt?.valid, // 7) SMT — شرط إلزامي بالمنهجية (مش تحسين اختياري)
    /* ترتيب زمني إلزامي: السعر لازم يوصل الـOB أولاً، وبعدين يصير الـSMT.
       بدون هالفحص ممكن يعتمد إعداد صار فيه SMT *قبل* ما يوصل السعر للـOB —
       وهاد تسلسل معكوس مش مطابق للمنهجية.
       منقارن بالوقت (ثواني) مش بالفهرس، لأنهم على فريمات مختلفة. */
    smtAfterOb: (() => {
      if (!smt?.valid || !ob?.eligible) return false;
      const smtTime = smt.eventA?.time;
      const obTime = ob.time;
      if (!Number.isFinite(smtTime) || !Number.isFinite(obTime)) return false;
      return smtTime >= obTime;
    })(),
    ...
    targetsCalculated: !!sequence?.active || !!nearestPeak || !!biggestSequence, // الأهداف محسوبة بسبب تحليلي واضح (سيكونز حالية، أو TP1 واقعي + أكبر سيكونز متوروشكا)
    // قاعدة SK رقم 1: "الدخول فقط من منطقة التصحيح ضمن المستويات الذهبية
    // (0.5-0.66)" المحسوبة من Origin إلى C. إشارة إضافية معروضة فقط حالياً
    // (مش داخلة بحساب Score لتفادي زعزعة الأوزان المعايرة أصلاً 100/100 قبل
    // ما نختبرها فعلياً على بيانات حية).
    priceInWCL: !!sequence?.priceInWCL,
    // الفصل ٩ (توثيق RADAR الجديد): "لا يسمح النظام بفتح أي صفقة أثناء صدور
    // الأخبار الاقتصادية المهمة، بغض النظر عن اكتمال باقي الشروط" — شرط إلزامي
    // يوقف الصفقة حتى لو باقي كل الشروط تحققت بالكامل.
    newsClear: !newsBlocked,
  };
  ...
  const mandatory = [
    ...
    checks.liquidityHit,
    checks.obCreated,
    checks.smtPresent, // SMT إلزامي
    checks.smtAfterOb, // ولازم يكون بعد وصول السعر للـOB
  ...
  // -------- Targets (خامس عشر + مبدأ المتوروشكا): TP1 = أقرب قمة/قاع فعلي
  // (سيولة قريبة واقعية)، TP2+ = أهداف أكبر سيكونز مؤكَّدة تاريخياً بنفس
  // الاتجاه (مش بالضرورة سيكونز الساق الحالية — أحياناً بيصير دخول من OB
  // بينما لسا ما تشكّلت سيكونز جديدة، فما لازم ننتظرها؛ منعتمد أكبر وحدة
  // موجودة أصلاً حسب مبدأ المتوروشكا). لو ولا وحدة متوفرة، نرجع لسلوك
  // sequence.targets الأصلي كما كان. --------
  let rawTargets;
  if (nearestPeak || biggestSequence) {
    const tp1 = nearestPeak
      ? { key: "TP1", ratio: null, price: nearestPeak.price, source: "nearest-peak" }
      : biggestSequence?.targets?.find((t) => t.key === "TP1") || null;
    const restFromBiggest = (biggestSequence?.targets || []).filter((t) => t.key !== "TP1");
    rawTargets = [tp1, ...restFromBiggest].filter(Boolean);
  } else {
    rawTargets = checks.targetsCalculated ? sequence.targets : [];
  }
  ...
  if (!checks.newsClear) reasonTags.push("News Block");
  ...
  const reasonsChecklist = [
    ...
    { key: "smtAfterOb", label: "SMT After OB Touch", ok: checks.smtAfterOb },
    ...
    { key: "priceInWCL", label: "Price In WCL Zone (0.5-0.66)", ok: checks.priceInWCL },
    { key: "newsClear", label: "No Major News Nearby", ok: checks.newsClear },
  ];
  ...
  if (checks.smtPresent && checks.smtAfterOb) radarWhy.push("SMT Confirmed (after OB)");
  else if (checks.smtPresent && !checks.smtAfterOb) radarWhy.push("SMT formed before price reached the OB — wrong order");
  else if (checks.obCreated) radarWhy.push("Awaiting SMT confirmation");
  ...
  if (!checks.newsClear) radarWhy.push(`News Block: ${newsBlocked?.title || "Major Event"} (${newsBlocked?.currency || ""})`);
  ...
  const result = {
    ...
    wcl: sequence?.wcl || null, // منطقة الدخول الذهبية (قاعدة SK رقم 1) — للعرض على الشارت
    reasonTags,
    reasonsChecklist,
    ...
    sequence,
    smt,
    newsBlock: newsBlocked || null, // الفصل ٩ — تفاصيل أقرب خبر مهم يمنع الدخول حالياً (أو null)
