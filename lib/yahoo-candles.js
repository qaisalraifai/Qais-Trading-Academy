...
export async function fetchYahooCandles(symbol, interval = "15min", wanted = 1000, anchorTimestamp = null) {
  ...
  try {
    ...
    /* نفس مشكلة الذهب عند Twelve Data بالضبط (شوفي lib/twelvedata-candles.js)
       بس هون عند Yahoo لأزواج الفوركس: السوق مقفول فعلياً السبت والأحد، بس
       يوهو أحياناً برجّع شمعة يومية لهيك أيام برضه (سعر شبه ثابت = فتح تقريباً
       يساوي إغلاق)، فبتطلع كـ"شحطة رفيعة" بدل شمعة طبيعية بالشارت (بالضبط
       الشكل يلي وصفته المستخدمة - خط رفيع بدون جسم واضح). TradingView
       والبروكرات ما بتعرض هيك شموع أصلاً (بتدمج عطلة الأسبوع بشمعة الإثنين).
       فبنشيلها بس لأزواج الفوركس (رمزها ينتهي بـ"=X" عند يوهو - شرط دقيق
       بيستثني المعادن =F/=، والكريبتو BTC-USD، والأسهم) وبس بالفريم اليومي
       (فريمات أقل من يوم إلها منطق تداول حقيقي مختلف بعطلة الأسبوع أحياناً). */
    if (interval === "1day" && /=X$/.test(symbol)) {
