...
export function permissionsForRole(role) {
  ...
  if (role === LIVE_ROLES.host) {
    return { ...base, canPublish: true, canPublishSources: ["camera", "microphone", "screen_share", "screen_share_audio"], roomAdmin: true, roomRecord: true };
  }
  if (role === LIVE_ROLES.moderator) {
    return { ...base, canPublish: true, canPublishSources: ["camera", "microphone", "screen_share", "screen_share_audio"], roomAdmin: false };
  }
  // الطالب فيه يفعّل كاميرا/مايك/مشاركة شاشة كمان (نمط اجتماع/صف تفاعلي)، بس بدون صلاحيات إدارية
  return { ...base, canPublish: true, canPublishSources: ["camera", "microphone", "screen_share", "screen_share_audio"] };
}
