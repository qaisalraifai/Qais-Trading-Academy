...
export async function getProfileBasics(supabase, userId) {
  ...
  return { username, isAdmin: profile?.role === "admin", daysLeft, gender: profile?.gender ?? null };
}
...
export async function getShellProfile(supabase, userId, options = {}) {
  ...
  /* صيغة المخاطبة بتمشي مع البروفايل لكل صفحة — الغياب null مش خطأ. */
  return { username, isAdmin, daysLeft, gender: profile?.gender ?? null, initials: (username || "؟").trim().charAt(0).toUpperCase() };
}
