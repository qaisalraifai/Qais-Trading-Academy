...
export const SESSION_PATHS = [
  ...
  "/admin",
  /* ⚠️ `/accounts` **صفحة إدارية** مع إنها جوّا مجموعة `(shell)`:
       app/(shell)/accounts/page.js → if (!shellProfile.isAdmin) redirect("/dashboard")

     كانت محطوطة بطبقة الاشتراك أول الجولة على أساس إنها محتوى أعضاء —
     وهاد غلط بالتصنيف. مش محتوى مدفوع، فحطّها هناك بيبعت طالباً بلا اشتراك
     لصفحة الدفع على صفحة **ما بيقدر يفتحها أصلاً حتى لو دفع**.
     الصح: جلسة هون، ودور الأدمن بينفرض بالصفحة نفسها.
     (الوحيدة من نوعها — مفحوصة على كل صفحات الشِل.) */
  "/accounts",
];
