...
export async function runMonthlyInfinityBonus(supabaseAdmin) {
  ...
  const infinityBonusPercent = await getSetting(supabaseAdmin, "infinity_bonus_percent");
  ...
  for (const leader of qualified) {
    ...
    const amount = Math.round(((totalTeamVolume * infinityBonusPercent) / 100) * 100) / 100;
